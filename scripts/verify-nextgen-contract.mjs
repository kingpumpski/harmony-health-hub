...
for (const token of ['evaluateIncomingEnvelope','classifyDeliveryFailure','nextRetry','canReplay']) if (!integration.includes(token)) throw new Error(`Integration runtime boundary missing: ${token}`);
...
console.log(`Next-gen contract verification passed: ${ids.length} modules; ${required.length} required artifacts; RLS, lifecycle, idempotency, safety, integration-runtime, AI-governance, deployment-profile, communication-policy and runtime-composition checks present.`);
