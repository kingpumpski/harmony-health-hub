...
try {
  execFileSync(process.platform === 'win32' ? 'npx.cmd' : 'npx', ['tsc','--target','ES2022','--module','commonjs','--moduleResolution','node','--skipLibCheck','--esModuleInterop','--outDir',temp,
    join(root,'src/lib/nextGenContracts.ts'), join(root,'src/lib/nextGenIntegrationRuntime.ts'), join(root,'src/lib/nextGenAIGovernance.ts'), join(root,'src/lib/nextGenCommunicationPolicy.ts'), join(root,'src/lib/nextGenDeploymentProfile.ts'), join(root,'src/lib/nextGenClinicalSafety.ts'), join(root,'src/lib/nextGenRuntimeGuards.ts')], { cwd: root, stdio: 'inherit' });
  ...
  const baseEnvelope = { messageId:'msg-1', correlationId:'corr-1', sourceSystem:'test-source', destinationSystem:'harmony', standard:'FHIR_R4', direction:'inbound', eventType:'Patient.updated', schemaVersion:'1.0', occurredAt:new Date().toISOString(), classification:'clinical', payload:{id:'p1'} };
  for (const standard of ['FHIR_R4','FHIR_R5','HL7_V2','DICOM','ASTM','REST_JSON','SOAP_XML']) if (!contracts.validateEnvelope({...baseEnvelope,messageId:`msg-${standard}`,standard})) throw new Error(`Supported interoperability standard rejected: ${standard}`);
  for (const invalid of [{standard:'NOT_A_STANDARD'},{direction:'sideways'},{classification:'restricted'},{occurredAt:'not-a-date'},{messageId:' '},{correlationId:''},{sourceSystem:' '},{eventType:''},{schemaVersion:''},{payload:undefined},{destinationSystem:42},{patientReference:42}]) if (contracts.validateEnvelope({...baseEnvelope,...invalid})) throw new Error(`Invalid interoperability envelope accepted: ${JSON.stringify(invalid)}`);
  ...
  if (integration.evaluateIncomingEnvelope({...baseEnvelope,payload:{id:'different'}},runtimeStore).decision !== 'quarantine') throw new Error('Message collision not quarantined');
  ...
  const failed1 = {messageId:'m',correlationId:'c',state:'failed',attemptCount:1};
  const retry = integration.nextRetry(failed1,{maxAttempts:5,retryBaseDelayMs:1000});
  ...
  if (integration.nextRetry({...failed1,attemptCount:5}).state !== 'quarantined') throw new Error('Maximum-attempt quarantine failed');
  if (integration.nextRetry({messageId:'m',correlationId:'c',state:'delivered',attemptCount:99}).state !== 'delivered') throw new Error('Delivered record rescheduled');
  if (integration.nextRetry({messageId:'m',correlationId:'c',state:'replayed',attemptCount:99}).state !== 'replayed') throw new Error('Replayed record rescheduled');
  if (integration.nextRetry({messageId:'m',correlationId:'c',state:'quarantined',attemptCount:5}).state !== 'quarantined') throw new Error('Quarantined record left terminal state');
  expectThrow(()=>integration.nextRetry({messageId:'m',correlationId:'c',state:'queued',attemptCount:0}),'Queued record bypassed state machine');
  expectThrow(()=>integration.nextRetry({messageId:'m',correlationId:'c',state:'failed',attemptCount:-1}),'Invalid attempt count accepted');
  expectThrow(()=>integration.nextRetry({...failed1},{maxAttempts:0,retryBaseDelayMs:1000}),'Invalid retry policy accepted');
  expectThrow(()=>integration.nextRetry({...failed1},{maxAttempts:5.5,retryBaseDelayMs:1000}),'Non-integer retry policy accepted');
  const replayable = {messageId:'m',correlationId:'c',state:'quarantined',attemptCount:5};
  if (integration.canReplay(replayable,{authorised:false,confirmed:true})) throw new Error('Unauthorized replay permitted');
  if (integration.canReplay(replayable,{authorised:true,confirmed:false})) throw new Error('Unconfirmed replay permitted');
  const pending = integration.prepareReplay(replayable,{authorised:true,confirmed:true});
  if (pending.state !== 'replay_pending' || !pending.nextAttemptAt) throw new Error('Replay transition failed');
  const replayed = integration.markReplayed(pending,{authorised:true,confirmed:true});
  if (replayed.state !== 'replayed' || replayed.attemptCount !== 6) throw new Error('Replay completion failed');
  expectThrow(()=>integration.markReplayed(replayable,{authorised:true,confirmed:true}),'Replay bypassed pending state');
  expectThrow(()=>integration.prepareReplay(pending,{authorised:true,confirmed:true}),'Replay pending state was re-entered');
  ...
  expectThrow(()=>ai.governClinicalOutput(model,{...output,intendedUse:'diagnosis'}),'Prohibited AI output accepted');
  expectThrow(()=>ai.governClinicalOutput({...model,lifecycle:'restricted'},output),'Restricted AI model accepted');
  expectThrow(()=>ai.governClinicalOutput(model,{...output,modelVersion:'2'}),'Unregistered model version accepted');
  expectThrow(()=>ai.governClinicalOutput(model,{...output,evidence:[]}),'AI output without evidence accepted');
  expectThrow(()=>ai.governClinicalOutput(model,{...output,confidence:1.1}),'Out-of-range AI confidence accepted');
  expectThrow(()=>ai.governClinicalOutput(model,{...output,provenance:{auditRequired:true}}),'Audited AI output without correlation accepted');
  ...
  if (communication.evaluateCommunicationRequest(prefs,{category:'marketing',channel:'sms'}).allowed) throw new Error('Unconsented marketing permitted');
  ...
  if (!operational.allowed || operational.language !== 'tw' || !operational.minimumNecessary) throw new Error('Operational minimum-necessary policy failed');
  if (communication.evaluateCommunicationRequest(prefs,{category:'emergency',channel:'email',emergency:true}).allowed) throw new Error('Emergency override bypassed authorization');
  ...
  if (!emergencyAllowed.allowed || !emergencyAllowed.minimumNecessary) throw new Error('Emergency override rejected');
  ...
  if (malformedQuiet.allowed || malformedQuiet.reason !== 'Invalid quiet-hours configuration') throw new Error('Malformed quiet-hours did not fail closed');
  ...
  const activeProfile = {profileKey:'gh',countryCode:'GH',regionCode:'GH-AA',locale:'en-GH',timezone:'Africa/Accra',currencyCode:'GHS',dataResidencyRegion:'GH',regulatoryProfile:'ghana',clinicalProfile:'default',communicationProfile:'default',accessibilityProfile:'wcag-2.2-aa',securityProfile:'zero-trust',enabledModules:['appointments','appointments'],effectiveFrom:'2026-01-01T00:00:00.000Z',effectiveTo:'2027-01-01T00:00:00.000Z'};
  ...
  expectThrow(()=>deployment.resolveDeploymentProfile(source,'gh',new Date('2025-12-01T00:00:00Z')),'Inactive profile accepted');
  expectThrow(()=>deployment.resolveDeploymentProfile(source,'gh',new Date('2027-01-01T00:00:00Z')),'Expired profile accepted at boundary');
  expectThrow(()=>deployment.resolveDeploymentProfile({findActive:()=>undefined},'gh',new Date()),'Missing profile accepted');

  for (const context of [{authorised:false,confirmed:true,audited:true,online:true},{authorised:true,confirmed:false,audited:true,online:true},{authorised:true,confirmed:true,audited:false,online:true},{authorised:true,confirmed:true,audited:true,online:false}]) expectThrow(()=>guards.guardClinicalAction('medication-administration',context),'Unsafe clinical action permitted');
  ...
  expectThrow(()=>guards.guardAIOutput({...model,evaluationPassed:false},output),'Unevaluated AI output permitted');
  expectThrow(()=>guards.guardAIOutput(model,{...output,intendedUse:'diagnosis'}),'Prohibited AI output permitted');
  expectThrow(()=>guards.guardDeploymentModule({findActive:()=>activeProfile},'gh','pharmacy',new Date('2026-06-01T00:00:00Z')),'Disabled module permitted');
  expectThrow(()=>guards.guardCommunication(prefs,{category:'marketing',channel:'sms'}),'Unconsented communication permitted');
