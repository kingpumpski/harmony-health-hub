...
const ROLE_MAP: Record<string, string[]> = {
  appointments: ['admin','practitioner','nurse','midwife','specialist_nurse','front_desk'], ward: ['admin','practitioner','nurse','midwife','specialist_nurse'], handover: ['admin','practitioner','nurse','midwife','specialist_nurse'], nursing_care: ['admin','practitioner','nurse','midwife','specialist_nurse'], theatre: ['admin','practitioner','nurse','specialist_nurse'], transfusion: ['admin','practitioner','nurse','midwife','specialist_nurse'], insurance: ['admin','accountant'], medication_administration: ['admin','nurse','specialist_nurse','midwife','practitioner'], ai_clinical: ['admin','practitioner','nurse','midwife','specialist_nurse','radiologist'], data_migration: ['admin'], facilities: ['admin','front_desk','accountant'],
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: roleRow, error: roleError } = await db.from('user_roles').select('role').eq('user_id', authData.user.id).maybeSingle();
    if (roleError) return json({ error: 'Unable to resolve staff role' }, 500);
    const role = String(roleRow?.role ?? '');
    if (!role) return json({ error: 'Staff role required' }, 403);
    ...
    if (module === 'handover') { const {data,error}=await (await q('nursing_shift_handovers','id,patient_id,shift_label,clinical_summary,pending_tasks,safety_concerns,escalation_required,acknowledged_at,created_at')).order('created_at',{ascending:false}).limit(limit); if(error) throw error; return json({handovers:data??[]}); }
    ...
    if (module === 'theatre') { const [c,p]=await Promise.all([(await q('theatre_cases','id,patient_id,procedure_name,theatre_name,scheduled_start,urgency,status,anesthetist_id')).order('scheduled_start',{ascending:true}).limit(limit),(await q('profiles','id,full_name,specialization')).order('full_name').limit(limit)]); if(c.error) throw c.error; if(p.error) throw p.error; return json({cases:c.data??[],profiles:p.data??[]}); }
  ...
  } catch (error) { console.error(error); return json({ error: 'Operational workspace query failed' }, 500); }
});
