...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (callerRolesError || !callerRoles?.some((r) => allowedRoles.includes(String(r.role)))) return new Response(JSON.stringify({ error: 'Forbidden' }), { status: 403, headers: { ...cors, 'Content-Type': 'application/json' } });
    ...
    const { error: queueError } = await db.from('notification_queue').insert(queueRows);
    if (queueError) throw queueError;
    const { error: notificationError } = await db.from('notifications').insert(roles.map((r) => ({ recipient_role: r, title: titleMsg, message: `${effectiveTitle} (${effectiveType}) analyzed by AI.${isUrgent ? ' Review immediately.' : ''}`, severity, category: 'lab', related_entity_id: documentId, related_patient_id: doc.patient_id, link: '/outside-lab' })));
