...
Deno.serve(async req=>{
 ...
 for(const row of rows??[]){const attempt=(row.attempts??0)+1,p=(row.payload??{}) as Record<string,unknown>;try{
   ...
   if(row.facility_id&&(!facilityConfig?.enabled||facilityConfig.kill_switch||facilityConfig.rollout_percent<=0)){await db.from('notification_queue').update({status:'cancelled',last_error:'Facility notification module is not enabled'}).eq('id',row.id);continue;}
