...
Deno.serve(async req=>{
 ...
 for(const row of rows??[]){const attempt=(row.attempts??0)+1,p=(row.payload??{}) as Record<string,unknown>;try{
   ...
   if(row.facility_id&&(!facilityConfig?.enabled||facilityConfig.kill_switch||!rolloutAllows(String(userId),String(row.facility_id),Number(facilityConfig.rollout_percent??0)))){await db.from('notification_queue').update({status:'failed',attempts:attempt,last_error:'Facility notification rollout or kill switch prevented delivery',updated_at:new Date().toISOString()}).eq('id',row.id).eq('status','processing');failed++;continue;}
