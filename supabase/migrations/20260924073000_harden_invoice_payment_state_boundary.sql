-- Keep invoice-item payment mutations bound to an open invoice.
...
BEGIN
  ...
  SELECT patient_id,status INTO invoice_patient_id,invoice_status
  FROM public.invoices
  WHERE id=_invoice_id
  FOR UPDATE;

  IF NOT FOUND THEN RAISE EXCEPTION 'Invoice not found'; END IF;
  IF invoice_patient_id IS NULL THEN RAISE EXCEPTION 'Invoice has no patient'; END IF;
  IF invoice_status NOT IN ('pending','partially_paid') THEN
    RAISE EXCEPTION 'Payments can only be applied to open invoices';
  END IF;
  ...
  VALUES (_invoice_id,invoice_patient_id,total,v_method,NULLIF(trim(_reference),''),uid,'Item-level payment')
  ...
  LOOP
    ...
    IF order_id IS NULL AND item.department IS NOT NULL THEN
      ...
      VALUES(invoice_patient_id,item.department,item.description,item.amount,item.source_id,_invoice_id,item.id,'pending_payment_approval',uid,item.service_code)
