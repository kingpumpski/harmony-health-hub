...
-- Existing governed import workflow: replace broad administrator checks with
-- the canonical role + facility module permission boundary. Import batches
-- remain the only path; this does not create direct writes to clinical truth.
...
BEGIN
  ...
  SELECT * INTO b FROM public.hms_import_batches WHERE id=_batch_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Import batch not found'; END IF;
  IF b.status<>'approved' THEN RAISE EXCEPTION 'Only approved batches may be committed'; END IF;
  FOR s IN SELECT * FROM public.hms_import_staging WHERE batch_id=_batch_id AND validation_status='pass' ORDER BY row_number FOR UPDATE LOOP
    v_count:=v_count+1;
  END LOOP;
  UPDATE public.hms_import_batches SET status='committed',committed_by=auth.uid(),committed_at=now(),accepted_count=v_count WHERE id=_batch_id;
  INSERT INTO public.hms_import_audit(batch_id,action,actor_id,details) VALUES(_batch_id,'committed',auth.uid(),jsonb_build_object('accepted',v_count));
  RETURN jsonb_build_object('batch_id',_batch_id,'accepted',v_count,'status','committed');
END
$$;

CREATE OR REPLACE FUNCTION public.rollback_hms_import_batch(_batch_id uuid,_reason text)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE b hms_import_batches%ROWTYPE; v_facility uuid;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  ...
  SELECT * INTO b FROM public.hms_import_batches WHERE id=_batch_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Import batch not found'; END IF;
  IF b.status NOT IN ('approved','committed','quarantined','validated') THEN RAISE EXCEPTION 'Batch cannot be rolled back from status %',b.status; END IF;
  UPDATE public.hms_import_batches SET status='rolled_back',rollback_at=now() WHERE id=_batch_id;
  INSERT INTO public.hms_import_audit(batch_id,action,actor_id,details) VALUES(_batch_id,'rolled_back',auth.uid(),jsonb_build_object('reason',_reason));
END
$$;
...
REVOKE ALL ON FUNCTION public.rollback_hms_import_batch(uuid,text) FROM PUBLIC,anon;
...
GRANT EXECUTE ON FUNCTION public.rollback_hms_import_batch(uuid,text) TO authenticated;

COMMENT ON FUNCTION public.hms_has_permission(uuid,text,text)
IS 'Canonical role-to-module permission check. Existing user_roles/app_role remains the user authority; facility module enablement is a mandatory second gate.';
COMMENT ON FUNCTION public.hms_assert_permission(uuid,text,text)
IS 'Fail-closed HMS permission boundary for protected workflows. UI visibility is advisory only.';
