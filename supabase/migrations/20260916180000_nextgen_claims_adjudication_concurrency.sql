...
BEGIN
  ...
  -- Terminal states are immutable except for an idempotent repeat of the same state.
  IF c.status IN ('paid','voided') THEN
    ...
    RETURN jsonb_build_object('claim_id', c.id, 'status', c.status,
      'amount_approved', c.amount_approved, 'amount_paid', c.amount_paid,
      'idempotent', TRUE);
  END IF;

  -- Explicit monotonic adjudication lifecycle; administrative void is allowed from
  -- every non-terminal state, while resubmission can only leave a rejected claim.
  CASE c.status
  ...
  IF NOT v_allowed THEN
    RAISE EXCEPTION 'Invalid claim transition from % to %', c.status, _status;
  END IF;
  ...
  IF v_approved IS NOT NULL AND v_approved > c.amount_claimed THEN
    RAISE EXCEPTION 'Approved amount exceeds claimed amount';
  END IF;
  IF v_paid > COALESCE(v_approved, c.amount_approved, 0) THEN
    RAISE EXCEPTION 'Paid amount exceeds approved amount';
  END IF;
  IF _status IN ('approved','partially_approved') AND v_approved IS NULL THEN
    RAISE EXCEPTION 'Approved amount is required for an adjudication decision';
  END IF;
  IF _status = 'paid' AND v_paid <= 0 THEN
    RAISE EXCEPTION 'Paid amount is required before marking a claim paid';
  END IF;
  ...
  RETURN jsonb_build_object('claim_id',c.id,'status',_status,
    'amount_approved',COALESCE(_amount_approved,c.amount_approved),
    'amount_paid',CASE WHEN _status='paid' THEN v_paid ELSE c.amount_paid END);
END;
$$;
...
BEGIN
  ...
  IF v_paid < 0 OR v_paid > COALESCE(v_approved,c.amount_approved,0) THEN
  ...
  INSERT INTO public.insurance_claim_events(claim_id,event_type,from_status,to_status,notes,actor_id)
  VALUES (c.id,'financials_updated',c.status,c.status,
    concat('approved=',COALESCE(_amount_approved::text,'unchanged'),';paid=',COALESCE(_amount_paid::text,'unchanged')),
    uid);
...
COMMENT ON FUNCTION public.transition_insurance_claim(UUID,TEXT,NUMERIC,NUMERIC,TEXT,TEXT) IS
  'Server-authoritative, row-locked insurance claim lifecycle and adjudication transition. Terminal states are immutable and every mutation records a canonical claim event.';
COMMENT ON FUNCTION public.update_insurance_claim_financials(UUID,NUMERIC,NUMERIC,TEXT,TEXT) IS
  'Server-authoritative, row-locked insurance claim financial mutation with amount bounds and canonical claim-event recording.';
