...
-- A failed replacement must never leave the target account without a role.
CREATE OR REPLACE FUNCTION public.admin_update_user_role(
  _target_user_id UUID,
  _next_role public.app_role
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF _target_user_id = v_actor AND _next_role <> 'admin'::public.app_role THEN
  ...
  VALUES (_target_user_id, _next_role);

  PERFORM public.record_system_audit(
    _action := 'admin_update_user_role',
    _module := 'administration',
    _entity_type := 'user',
    _entity_id := _target_user_id,
    _severity := 'info',
    _metadata := jsonb_build_object(
      'assigned_role', _next_role,
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'user', jsonb_build_object('id', _target_user_id, 'role', _next_role)
  );
END;
$$;
