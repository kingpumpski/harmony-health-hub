-- Tighten authenticated RPC boundaries identified during SECURITY DEFINER review.
-- Keep the client-facing workflow RPCs available, but require an established
-- operational role before they can create billable clinical orders.
...
BEGIN
  ...
  END IF;
  IF _patient_id IS NULL OR NULLIF(trim(_study_name),'') IS NULL THEN RAISE EXCEPTION 'Patient and study name are required'; END IF;
...
BEGIN
  ...
  IF NOT (
    ...
    public.has_role(v_uid,'nurse') OR
    public.has_role(v_uid,'midwife') OR
    public.has_role(v_uid,'lab_technician') OR
    public.has_role(v_uid,'front_desk')
  ) THEN
...
-- Audit entries are written by trusted SECURITY DEFINER workflow functions and
-- triggers. They must not be directly forgeable through a client RPC.
REVOKE EXECUTE ON FUNCTION public.record_system_audit(text,text,text,uuid,text,jsonb) FROM anon, authenticated, PUBLIC;
