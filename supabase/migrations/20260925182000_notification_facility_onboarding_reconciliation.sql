...
-- Provider credentials remain outside PostgreSQL; only secret references and operational metadata are stored.
  ...
  IF EXISTS (
    ...
    WHERE code <> 'in_app' AND is_enabled='true'
      AND NOT EXISTS (
        ...
        WHERE pc.facility_id=_facility_id AND pc.channel=enabled.code
          AND pc.environment IN ('test','production') AND pc.status='verified'
      )
  ) THEN
