-- Restore the full operational workspace dispatcher while retaining facility-aware ward reads.
 ...
 ELSIF _module='ward' THEN
  ...
  SELECT jsonb_build_object(
    'wards',COALESCE((SELECT jsonb_agg(to_jsonb(x)) FROM (
      ...
      FROM ward_units
      WHERE active AND (v_role='admin' OR facility_id IS NULL OR facility_id=v_facility)
