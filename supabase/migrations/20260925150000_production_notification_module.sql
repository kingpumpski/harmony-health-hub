...
CREATE OR REPLACE FUNCTION public.enqueue_notification_v2(
  ...
  _tenant_id UUID DEFAULT NULL, _locale TEXT DEFAULT NULL, _timezone TEXT DEFAULT NULL
)
...
BEGIN
  ...
  VALUES(COALESCE(_channels->>0,'in_app'), _payload, 'pending', idem, _tenant_id, _user_id, _priority, _event_name, _template_key, _locale, _timezone, COALESCE(_scheduled_for,now()), COALESCE(_channels,'["in_app"]'::jsonb), COALESCE(_scheduled_for,now()))
