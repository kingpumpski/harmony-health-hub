-- Enforce generation lifecycle transitions and accounting integrity at the database boundary.
-- This complements RLS: authorized users may only move runs/items through supported
-- states, and terminal run counts must agree with the generated item states.
...
BEGIN
  ...
  IF OLD.status = 'processing' AND NEW.status NOT IN ('processing', 'completed', 'warning', 'failed') THEN
    RAISE EXCEPTION 'Invalid report generation item transition.' USING errcode = '22000';
  END IF;
  IF OLD.status IN ('completed', 'warning', 'failed') AND NEW.status <> OLD.status THEN
...
CREATE TRIGGER trg_validate_report_generation_item_transition
BEFORE UPDATE ON public.report_generation_items
FOR EACH ROW EXECUTE FUNCTION public.validate_report_generation_item_transition();
