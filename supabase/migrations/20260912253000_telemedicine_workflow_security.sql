-- Telemedicine production workflow hardening.
-- Scheduling, payment confirmation and lifecycle transitions are server-authorized.
...
BEGIN
  ...
  WHERE video_sessions.id = _session_id
    AND practitioner_id = auth.uid()
    AND status IN ('scheduled','ready')
    AND (NOT payment_required OR payment_received);

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Session is unavailable, not assigned to you, or payment is outstanding';
  END IF;
