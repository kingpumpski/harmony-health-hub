-- Link reconciled discharge billing to the existing insurance-claim lifecycle.
-- Additive and idempotent: never creates a duplicate claim for the same invoice/payer.
...
BEGIN
  ...
  IF v_patient_id IS NULL THEN RAISE EXCEPTION 'Discharge billing handoff not found'; END IF;

  SELECT r.invoice_id, r.outstanding
    INTO v_invoice_id, v_amount
  ...
  SELECT COALESCE(NULLIF(trim(_payer_name),''), p.insurance_provider)
    INTO v_payer
  FROM public.patients p WHERE p.id = v_patient_id;

  IF NULLIF(trim(v_payer),'') IS NULL THEN
    RAISE EXCEPTION 'Insurance payer is required';
  END IF;

  SELECT id INTO v_existing
  FROM public.insurance_claims
  WHERE invoice_id = v_invoice_id
    AND payer_name = v_payer
    AND status <> 'voided'
  ...
  SELECT public.create_insurance_claim_draft(
    v_patient_id,
    v_payer,
    _member_number,
    v_amount,
  ...
  PERFORM public.record_system_audit(
    'discharge_insurance_claim_created',
    'insurance',
    'insurance_claim',
    v_claim_id,
    'info',
...
REVOKE ALL ON FUNCTION public.create_discharge_insurance_claim(UUID,TEXT,TEXT) FROM PUBLIC;
