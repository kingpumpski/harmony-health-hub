-- Allow admission immediately after an encounter is successfully saved.
-- Treatment-before-deposit remains controlled by facility financial override settings.
...
BEGIN
  ...
  SELECT COALESCE(allow_treatment_before_deposit,FALSE)
    ...
    INTO v_override
  FROM public.facility_settings ORDER BY created_at LIMIT 1;
  ...
  IF v_override THEN
    FOR v_order IN SELECT * FROM public.service_orders
      WHERE encounter_id=v_enc.id AND status='pending_payment_approval' FOR UPDATE
    LOOP
      INSERT INTO public.billing_overrides(
        service_order_id,patient_id,department,related_entity_id,reason,
        overridden_by,approved_by,approved_at
      ) VALUES(
      ...
      ON CONFLICT(service_order_id) DO UPDATE SET
        reason=EXCLUDED.reason,overridden_by=EXCLUDED.overridden_by,
        approved_by=EXCLUDED.approved_by,approved_at=EXCLUDED.approved_at;

      UPDATE public.service_orders SET
        status='released',approved_at=now(),approved_by=auth.uid(),
