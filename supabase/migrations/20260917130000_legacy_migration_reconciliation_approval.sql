...
CREATE OR REPLACE FUNCTION public.reconcile_legacy_migration_row(
  _row_id UUID,
  _patient_id UUID,
  _match_confidence NUMERIC DEFAULT NULL,
  _match_method TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
  v_row public.data_migration_rows;
  v_conf NUMERIC := COALESCE(_match_confidence, 0);
BEGIN
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Migration row not found'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.data_migration_batches WHERE id = v_row.batch_id AND status IN ('staged','validating','ready')) THEN
    RAISE EXCEPTION 'Migration batch is not open for reconciliation';
  END IF;

  UPDATE public.data_migration_rows
  SET patient_id = _patient_id,
      match_confidence = v_conf,
      match_method = COALESCE(NULLIF(btrim(_match_method),''),'manual'),
  ...
  PERFORM public.record_system_audit(
    'legacy_migration_row_reconciled','data_migration','data_migration_row',_row_id,'info',
    jsonb_build_object('batch_id',v_row.batch_id,'patient_id',_patient_id,'match_confidence',v_conf,'match_method',COALESCE(_match_method,'manual'))
  );
...
BEGIN
  ...
  FOR v_row IN SELECT * FROM public.data_migration_rows WHERE batch_id=_batch_id ORDER BY source_row_number FOR UPDATE LOOP
    ...
    IF jsonb_array_length(v_errors) = 0 THEN
      UPDATE public.data_migration_rows SET row_status='accepted', validation_errors='[]'::jsonb WHERE id=v_row.id;
      v_valid := v_valid + 1;
    ELSE
      UPDATE public.data_migration_rows SET row_status='rejected', validation_errors=v_errors, rejection_reason=COALESCE(v_errors->>0,'Validation failed') WHERE id=v_row.id;
      v_rejected := v_rejected + 1;
    END IF;
  END LOOP;

  UPDATE public.data_migration_batches
  SET accepted_rows=v_valid,
      rejected_rows=v_rejected,
      validation_errors=(SELECT COALESCE(jsonb_agg(jsonb_build_object('row_id',id,'source_row_number',source_row_number,'errors',validation_errors)),'[]'::jsonb) FROM public.data_migration_rows WHERE batch_id=_batch_id AND row_status='rejected'),
      status=CASE WHEN v_total > 0 AND v_valid=v_total THEN 'ready' ELSE 'staged' END
  WHERE id=_batch_id;
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE
  v_batch public.data_migration_batches;
  v_row RECORD;
  v_count INTEGER := 0;
  v_skipped INTEGER := 0;
  v_legacy_id UUID;
...
BEGIN
  ...
  FOR v_row IN SELECT r.*, b.source_system AS batch_source_system FROM public.data_migration_rows r JOIN public.data_migration_batches b ON b.id=r.batch_id WHERE r.batch_id=_batch_id AND r.row_status='accepted' ORDER BY r.source_row_number FOR UPDATE LOOP
    BEGIN
      v_occurred_at := NULLIF(v_row.raw_data->>'occurred_at','')::timestamptz;
