-- Replace remaining RLS references to the intentionally non-client-callable
-- is_clinical_staff(UUID) helper with the current-user-only helper.
-- This preserves the existing authorization semantics without reopening
-- arbitrary-user role probing through the Data API.

DROP POLICY IF EXISTS acm_clinical_all ON public.ai_case_memory;
CREATE POLICY acm_clinical_all ON public.ai_case_memory
...
CREATE POLICY clinical_staff_manage_ai_diagnosis_suggestions ON public.ai_diagnosis_suggestions
  FOR ALL TO authenticated
  USING (public.current_user_is_clinical_staff())
  WITH CHECK (public.current_user_is_clinical_staff());

DROP POLICY IF EXISTS ap_clinical_read ON public.ai_protocols;
CREATE POLICY ap_clinical_read ON public.ai_protocols
  FOR SELECT TO authenticated
  USING (public.current_user_is_clinical_staff());

DROP POLICY IF EXISTS air_clinical_all ON public.ai_report_requests;
CREATE POLICY air_clinical_all ON public.ai_report_requests
  FOR ALL TO authenticated
  USING (public.current_user_is_clinical_staff())
  WITH CHECK (public.current_user_is_clinical_staff());

DROP POLICY IF EXISTS staff_read_symptom_map ON public.ai_symptom_icd_map;
CREATE POLICY staff_read_symptom_map ON public.ai_symptom_icd_map
  FOR SELECT TO authenticated
  USING (public.current_user_is_clinical_staff());
...
CREATE POLICY staff_manage_inpatient_reviews ON public.inpatient_reviews
  FOR ALL TO authenticated
  USING (public.current_user_is_clinical_staff())
  WITH CHECK (public.current_user_is_clinical_staff());
