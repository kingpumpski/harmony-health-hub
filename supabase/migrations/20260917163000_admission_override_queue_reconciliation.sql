-- Reconcile emergency admission with the canonical service-order workflow.
-- Treatment-before-deposit remains administrator-controlled through the canonical facility_configuration table.
-- This migration is repository-only until the approved production migration process is used.
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtextextended('hims:admission-encounter:' || _encounter_id::text, 0));

  IF EXISTS (
    SELECT 1 FROM public.admissions
    WHERE patient_id=v_enc.patient_id AND status='admitted' AND discharged_at IS NULL
  ) THEN
  ...
  VALUES(
    v_enc.patient_id,v_enc.id,NULLIF(btrim(_ward),''),
