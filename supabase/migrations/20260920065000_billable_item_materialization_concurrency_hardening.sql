-- Serialize bill preparation per patient and prevent concurrent open-invoice creation.
...
BEGIN
  ...
  -- Serialize all bill materialization for this patient. This closes the race
  -- where two simultaneous requests both observe no open invoice and create two ledgers.
  ...
  IF inv IS NULL THEN
    ...
    VALUES('INV-'||to_char(clock_timestamp(),'YYYYMMDDHH24MISSMS')||'-'||substr(gen_random_uuid()::text,1,6),_patient_id,0,uid)
    RETURNING id INTO inv;
  END IF;

  SELECT amount INTO tariff FROM public.service_tariffs WHERE service_code='CONSULTATION' AND active LIMIT 1;
  ...
  SELECT amount INTO tariff FROM public.service_tariffs WHERE service_code='LAB-GENERIC' AND active LIMIT 1;
  IF tariff IS NOT NULL THEN
  ...
  SELECT amount INTO tariff FROM public.service_tariffs WHERE service_code='PROCEDURE-GENERIC' AND active LIMIT 1;
  ...
  SELECT amount INTO tariff FROM public.service_tariffs WHERE service_code='WARD-ACCOM' AND active LIMIT 1;
