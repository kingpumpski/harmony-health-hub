-- Persist the authorization basis for service-order releases so clients do not need
-- a second billing_overrides query to distinguish payment from financial override.
...
BEGIN
  ...
  ) THEN RAISE EXCEPTION 'Accounts release permission required'; END IF;
  ...
  IF v_order.payment_required AND v_order.amount > 0
  ...
  SET status='released',
      ...
      release_reason=CASE
        WHEN v_override THEN 'Emergency financial override: ' || COALESCE(NULLIF(_reason,''),'Approved financial override')
        ELSE COALESCE(NULLIF(_reason,''),'Payment received')
      END,
      updated_at=now()
  ...
  INSERT INTO public.department_queues(
    service_order_id, patient_id, department, related_encounter_id, related_invoice_id,
    payment_required, payment_satisfied, priority, reason, created_by, queued_at, status
  )
  VALUES(
    v_order.id, v_order.patient_id, v_order.department, v_order.encounter_id, v_order.invoice_id,
    v_order.payment_required, true, 'normal', v_order.service_name, auth.uid(), now(), 'queued'
  )
  ...
  IF v_order.order_type='imaging' AND v_order.related_entity_id IS NOT NULL THEN
    UPDATE public.imaging_orders SET status='released', updated_at=now()
    WHERE id=v_order.related_entity_id AND service_order_id=v_order.id AND status='pending_payment_approval';
  END IF;
