...
BEGIN
  ...
  IF NOT (
    ...
    RAISE EXCEPTION 'Dental clinical role required';
  END IF;
  ...
  INSERT INTO public.dental_records (
    ...
    _patient_id,
    NULLIF(trim(_examination), ''),
    NULLIF(trim(_treatment_plan), ''),
    NULLIF(trim(_procedures_performed), ''),
    auth.uid()
  )
...
BEGIN
  ...
  IF COALESCE(_charge_amount, 0) > 0 THEN
    IF _service_order_id IS NULL THEN
      RAISE EXCEPTION 'A released service order is required for a chargeable procedure';
    END IF;
    ...
    IF NOT EXISTS (
      ...
      WHERE id = _service_order_id
        ...
        AND status IN ('released', 'in_progress', 'completed')
    ) THEN
      RAISE EXCEPTION 'Procedure payment has not been released';
    END IF;
  END IF;

  INSERT INTO public.procedure_notes (
    patient_id,
    procedure_name,
    template_used,
    indication,
    ...
    _patient_id,
    NULLIF(trim(_procedure_name), ''),
    NULLIF(trim(_template_used), ''),
    NULLIF(trim(_indication), ''),
    NULLIF(trim(_technique), ''),
    NULLIF(trim(_findings), ''),
    NULLIF(trim(_complications), ''),
    NULLIF(trim(_post_op_plan), ''),
    auth.uid(),
    'completed',
    GREATEST(COALESCE(_charge_amount, 0), 0),
    _service_order_id
  )
  RETURNING id INTO _id;
