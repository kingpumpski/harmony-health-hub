...
BEGIN
 ...
 SELECT jsonb_build_object('patient',v_patient,'appointments',COALESCE((SELECT jsonb_agg(to_jsonb(x)) FROM (SELECT a.* FROM public.appointments a WHERE a.patient_id=_patient_id ORDER BY a.scheduled_at DESC LIMIT 25) x),'[]'::jsonb),'vitals',COALESCE((SELECT jsonb_agg(to_jsonb(x)) FROM (SELECT v.* FROM public.vital_signs v WHERE v.patient_id=_patient_id ORDER BY v.recorded_at DESC LIMIT 25) x),'[]'::jsonb),'triage',COALESCE((SELECT jsonb_agg(to_jsonb(x)) FROM (SELECT t.* FROM public.triage_assessments t WHERE t.patient_id=_patient_id ORDER BY t.created_at DESC LIMIT 25) x),'[]'::jsonb),'encounters',COALESCE((SELECT jsonb_agg(to_jsonb(x)) FROM (SELECT e.* FROM public.encounters e WHERE e.patient_id=_patient_id ORDER BY e.created_at DESC LIMIT 25) x),'[]'::jsonb)) INTO v_result;
 RETURN v_result;
END; $$;
