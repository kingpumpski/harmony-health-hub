...
type ContinuityData = Record<string, ContinuityRow[]>;
...
function asRows(value: unknown): ContinuityRow[] {
  return Array.isArray(value) ? value.filter((row): row is ContinuityRow => Boolean(row && typeof row === 'object' && 'id' in row)) : [];
}
...
export default function PatientCareContinuity() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      add(asRows(continuity.transfusion), 'transfusion', (r) => ({ id: r.id, kind, title: `Transfusion: ${String(r.blood_product ?? 'Blood product')}`, status: r.status ? String(r.status) : null, detail: `${String(r.blood_group ?? 'Group not recorded')}${r.reaction_observed ? ' · Reaction observed' : ''}`, at: String(r.start_at ?? r.created_at ?? '') }));
      ...
      const message = error instanceof Error ? error.message : 'Unable to load continuity record';
  ...
  const patientRecordId = String(patient.id ?? patientId ?? '');
