  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={createCase} className="rounded-xl border bg-card p-4 grid gap-3 lg:grid-cols-[1.3fr_2fr_1fr_1fr_auto]">
        ...
        <input aria-label="Chief complaint or presenting problem" required value={form.chiefComplaint} onChange={(e) => setForm({ ...form, chiefComplaint: e.target.value })} placeholder="Chief complaint / presenting problem" className="w-full rounded-md border bg-background p-2 text-sm" />
        <select aria-label="Emergency acuity" value={form.acuity} onChange={(e) => setForm({ ...form, acuity: e.target.value })} className="w-full rounded-md border bg-background p-2 text-sm">{acuities.map((item) => <option key={item}>{item}</option>)}</select>
        <select aria-label="Arrival mode" value={form.arrivalMode} onChange={(e) => setForm({ ...form, arrivalMode: e.target.value })} className="w-full rounded-md border bg-background p-2 text-sm"><option value="walk_in">Walk-in</option><option value="ambulance">Ambulance</option><option value="referral">Referral</option><option value="other">Other</option></select>
        <button type="submit" disabled={busy} className="rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50">{busy ? 'Saving…' : 'Register case'}</button>
      </form>
      <div className="grid gap-3">{rows.map((r) => <article key={r.id} className="rounded-xl border bg-card p-4"><div className="flex flex-wrap justify-between gap-3"><div className="min-w-0"><h2 className="font-semibold truncate">{patient(r.patient_id)}</h2><p className="text-sm break-words">{r.chief_complaint}</p><p className="text-xs text-muted-foreground">{r.acuity} · {r.arrival_mode || 'Not recorded'} · {r.status} · {new Date(r.arrival_time).toLocaleString()}</p></div><div className="flex flex-wrap gap-2 w-full sm:w-auto"><button type="button" aria-label={`Assign ${patient(r.patient_id)} to me`} disabled={busy || !!r.assigned_officer} onClick={() => void assign(r.id)} className="rounded-md border px-2 py-1 text-sm disabled:opacity-50">{r.assigned_officer ? 'Assigned' : 'Assign to me'}</button><select aria-label={`Change status for ${patient(r.patient_id)}`} disabled={busy || ['discharged', 'referred', 'left_without_being_seen', 'cancelled'].includes(r.status)} value="" onChange={(e) => { if (e.target.value) void transition(r.id, e.target.value); }} className="w-full sm:w-auto rounded-md border bg-background px-2 py-1 text-sm"><option value="">Change status…</option>{statuses.filter((s) => s !== r.status).map((s) => <option key={s} value={s}>{s.replaceAll('_', ' ')}</option>)}</select></div></div></article>)}{!rows.length && <p className="text-sm text-muted-foreground">No emergency cases found.</p>}</div>
    </div>
  );
}
