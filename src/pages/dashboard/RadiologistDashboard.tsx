...
export default function RadiologistDashboard() {
  ...
  const load = async (announce = false) => {
    ...
    setOrders(next);
    setUnreadAlerts(notifications?.length ?? 0);
    setLoading(false);
  };
  ...
  useEffect(() => {
    const channel = supabase.channel('radiologist-dashboard-live')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'notifications' }, (payload) => {
        if (payload.eventType === 'INSERT' && String((payload.new as { severity?: string }).severity ?? '').toLowerCase() === 'critical') playWorkflowSound('critical');
        void load();
      })
  ...
  const counters = useMemo<Record<StatusKey, number>>(() => ({
    awaiting: orders.filter((o) => ['pending_payment_approval', 'pending_payment'].includes(o.status)).length,
    ready: orders.filter((o) => ['released', 'queued'].includes(o.status)).length,
    in_progress: orders.filter((o) => o.status === 'in_progress').length,
    completed: orders.filter((o) => o.status === 'completed').length,
    urgent: orders.filter((o) => ['urgent', 'stat'].includes(o.priority) && o.status !== 'completed').length,
  }), [orders]);

  const activeQueue = useMemo(() => orders.filter((o) => o.status !== 'completed').slice(0, 8), [orders]);
  const cards = [
    { key: 'awaiting' as StatusKey, label: 'Awaiting Accounts', icon: Clock3, tone: 'text-warning', surface: 'bg-warning/5', href: '/radiology' },
    { key: 'ready' as StatusKey, label: 'Ready for Imaging', icon: ImageIcon, tone: 'text-info', surface: 'bg-info/5', href: '/radiology' },
    { key: 'in_progress' as StatusKey, label: 'In Progress', icon: Loader2, tone: 'text-primary', surface: 'bg-primary/5', href: '/radiology' },
    { key: 'completed' as StatusKey, label: 'Completed', icon: CheckCircle2, tone: 'text-success', surface: 'bg-success/5', href: '/radiology' },
    { key: 'urgent' as StatusKey, label: 'Urgent / STAT', icon: AlertTriangle, tone: 'text-critical', surface: 'bg-critical/5', href: '/radiology' },
  ];

  return <div className="space-y-6 animate-fade-in">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div><h1 className="flex items-center gap-2 text-2xl font-heading font-bold"><ImageIcon className="h-6 w-6 text-primary" /> Radiologist Dashboard</h1><p className="text-muted-foreground">Live diagnostic imaging worklist, reporting and urgent-case monitoring.</p></div>
      <div className="flex items-center gap-2"><Link to="/notifications" className="btn-secondary inline-flex items-center gap-2"><BellRing className="h-4 w-4" /> Alerts {unreadAlerts > 0 && <span className="rounded-full bg-critical px-2 py-0.5 text-xs text-critical-foreground">{unreadAlerts}</span>}</Link><button type="button" onClick={() => { playWorkflowSound('info'); void load(); }} className="btn-secondary inline-flex items-center gap-2"><RefreshCw className="h-4 w-4" />{loading ? 'Refreshing…' : 'Refresh'}</button></div>
    </header>

    <section className="grid grid-cols-2 gap-3 lg:grid-cols-5">
      {cards.map((card) => { const Icon = card.icon; return <Link key={card.key} to={card.href} className={`card-medical p-4 transition-all hover:-translate-y-1 ${card.surface}`}><div className="flex items-center justify-between"><p className="text-xs text-muted-foreground">{card.label}</p><Icon className={`h-4 w-4 ${card.tone}`} /></div><p className={`mt-1 text-3xl font-bold ${card.tone} ${counters[card.key] > 0 && card.key !== 'completed' ? 'animate-pulse' : ''}`}>{counters[card.key]}</p><p className="mt-1 text-xs text-muted-foreground">Open worklist</p></Link>; })}
    </section>
