...
function PatientEncounterHistory({ patientId, currentEncounterId }: { patientId: string; currentEncounterId?: string }) {
  ...
  const loadHistory = async () => {
    ...
    const prior = (encounters ?? []).filter((item) => item.id !== currentEncounterId) as Encounter[];
    if (!prior.length) {
      setHistory([]);
      setLoading(false);
      return;
    ...
    setHistory(prior.map((item) => ({ ...item, diagnoses: byEncounter.get(item.id) ?? [] })));
    setLoading(false);
  };

  useEffect(() => { loadHistory(); }, [patientId, currentEncounterId]);
  ...
  return (
    <aside className="card-medical p-5 space-y-4 border-l-4 border-l-primary/60 lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="font-semibold flex items-center gap-2"><History className="w-4 h-4 text-primary" /> Clinical history at a glance</h3>
          <p className="text-xs text-muted-foreground mt-1">Previous findings shown here so the attending officer does not have to leave the encounter.</p>
        </div>
        ...
        <>
          {keyFindings.length > 0 && (
            <section className="rounded-xl border border-warning/40 bg-warning/5 p-4">
              <div className="flex items-center gap-2 font-semibold text-sm mb-2 text-warning-foreground">
                <AlertTriangle className="w-4 h-4" /> Conditions to notice
              </div>
              <div className="flex flex-wrap gap-2">
                {keyFindings.map((finding) => (
                  <span key={finding} className="rounded-full bg-background border border-warning/40 px-2.5 py-1 text-xs font-medium">
                    {finding}
                  </span>
                ))}
              </div>
          ...
          <section>
            <h4 className="text-sm font-semibold mb-2 flex items-center gap-2"><HeartPulse className="w-4 h-4" /> Previous encounters</h4>
            <div className="space-y-3">
              {history.map((item) => (
                <article key={item.id} className="rounded-xl border border-border p-3 bg-background/70">
                  <div className="flex items-start justify-between gap-3">
                    <span className="text-xs text-muted-foreground">{new Date(item.created_at).toLocaleString()}</span>
                    <span className="text-xs rounded-full bg-muted px-2 py-0.5">{item.status}</span>
                  </div>
                  <p className="text-sm font-semibold mt-2">
                    {item.principal_diagnosis || item.diagnoses.find((d) => d.is_principal)?.diagnosis || 'Clinical encounter'}
                  </p>
                  {item.diagnoses.length > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">
                      Other diagnoses: {item.diagnoses.filter((d) => !d.is_principal).map((d) => d.diagnosis).join(', ') || 'None recorded'}
                    </p>
                  )}
                  {item.symptoms && (
                    <p className="text-xs mt-2"><span className="font-medium">Presentation:</span> {item.symptoms}</p>
                  )}
                  {item.clerking_notes && (
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-3"><span className="font-medium text-foreground">Key note:</span> {item.clerking_notes}</p>
                  )}
                  {item.treatment_plan && (
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-2"><span className="font-medium text-foreground">Previous plan:</span> {item.treatment_plan}</p>
                  )}
                </article>
              ))}
            </div>
...
export default function Encounters() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-heading font-bold flex items-center gap-2"><Stethoscope className="w-6 h-6 text-primary" />Clinical Encounters</h1>
        <p className="text-muted-foreground">Document consultations with the patient’s key previous clinical findings visible alongside the current encounter.</p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
          ...
          <div className="card-medical p-5">
            ...
            <div className="space-y-2 max-h-[480px] overflow-auto">
              {encounters.map((e) => {
                ...
                return <button key={e.id} onClick={() => setSelected(e)} className={`w-full text-left rounded-xl border p-3 transition ${selected?.id === e.id ? 'border-primary bg-primary/5' : 'border-border hover:bg-accent/40'}`}>
                  <div className="flex justify-between items-start"><div><p className="font-medium text-sm">{p ? `${p.first_name} ${p.last_name}` : 'Patient'}</p><p className="text-xs text-muted-foreground">{new Date(e.created_at).toLocaleString()}</p></div><span className={`text-xs px-2 py-0.5 rounded-full ${e.status === 'completed' ? 'bg-success/15 text-success' : 'bg-warning/15 text-warning'}`}>{e.status}</span></div>
        ...
        <div className="space-y-6">
          ...
          <div className="card-medical p-6 min-h-[400px]">
            {!selected ? <div className="h-full flex items-center justify-center text-muted-foreground">Select or create an encounter to begin.</div> : (
              <div className="space-y-6">
                <div className="flex justify-between items-start">
                  <div><h2 className="text-lg font-semibold flex items-center gap-2"><FileText className="w-5 h-5" /> Encounter</h2><p className="text-sm text-muted-foreground">Started {new Date(selected.created_at).toLocaleString()}</p></div>
                  {selected.status !== 'completed' && <button onClick={completeEncounter} className="btn-primary inline-flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Complete</button>}
                </div>
                ...
                <section>
                  <h3 className="font-semibold mb-2 flex items-center gap-2"><FlaskConical className="w-4 h-4" /> Diagnoses</h3>
                  <div className="flex gap-2 mb-3"><input value={newDx} onChange={(e) => setNewDx(e.target.value)} className="input-medical flex-1" placeholder="Add diagnosis (e.g. Malaria)" /><button onClick={addDiagnosis} className="btn-primary">Add</button></div>
                ...
                <section>
                  ...
                  <form onSubmit={addPrescription} className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3"><input value={med} onChange={(e) => setMed(e.target.value)} className="input-medical" placeholder="Medication" /><input value={dose} onChange={(e) => setDose(e.target.value)} className="input-medical" placeholder="Dose" /><input value={freq} onChange={(e) => setFreq(e.target.value)} className="input-medical" placeholder="Frequency" /><input value={duration} onChange={(e) => setDuration(e.target.value)} className="input-medical" placeholder="Duration" /><button className="btn-primary col-span-2 md:col-span-4">Prescribe</button></form>
                  <div className="space-y-2">{prescriptions.map((p) => <div key={p.id} className="rounded-xl border border-border p-3 text-sm flex justify-between items-center"><div><span className="font-medium">{p.medication}</span><span className="text-muted-foreground"> · {p.dosage} · {p.frequency} · {p.duration}</span></div><span className={`text-xs px-2 py-0.5 rounded-full ${p.status === 'dispensed' ? 'bg-success/15 text-success' : 'bg-warning/15 text-warning'}`}>{p.status}</span></div>)}{prescriptions.length === 0 && <p className="text-sm text-muted-foreground">No prescriptions yet.</p>}</div>
                </section>
              </div>
            )}
          </div>
