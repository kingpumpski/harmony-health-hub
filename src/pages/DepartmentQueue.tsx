  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2"><span className="inline-flex items-center gap-1 rounded-full bg-primary/5 px-3 py-1 text-xs text-muted-foreground"><BellRing className="w-3.5 h-3.5" /> Live queue</span><button onClick={() => void load()} className="btn-secondary inline-flex items-center gap-2 self-start"><RefreshCw className="w-4 h-4" /> Refresh</button></div>
      </div>
      {department && <div className="grid grid-cols-2 sm:grid-cols-3 gap-3"><div className="card-medical p-4 bg-warning/5"><p className="text-xs text-muted-foreground">Active patients</p><p className="text-3xl font-bold tabular-nums animate-fade-in">{rows.length}</p></div><div className="card-medical p-4 bg-primary/5"><p className="text-xs text-muted-foreground">Queued</p><p className="text-3xl font-bold tabular-nums">{rows.filter((row) => row.status === 'queued').length}</p></div><div className="card-medical p-4 bg-info/5"><p className="text-xs text-muted-foreground">In progress</p><p className="text-3xl font-bold tabular-nums">{rows.filter((row) => row.status === 'claimed').length}</p></div></div>}
      ...
      {department && rows.length === 0 && <div className="card-medical p-8 text-center text-sm text-muted-foreground">No released service orders are waiting for your department.</div>}
