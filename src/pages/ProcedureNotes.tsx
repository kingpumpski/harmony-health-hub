...
export default function ProcedureNotes() {
  ...
  useEffect(() => {
    void supabase.from('patients').select('id, first_name, last_name').limit(200).then(({ data }) => setPatients((data ?? []) as Patient[]));
    void load();
  }, []);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (chargeAmount > 0) {
        ...
        toast({ title: 'Payment approval required', description: 'The procedure was sent to Accounts and remains blocked until released.' });
        resetForm();
        return;
      }
      ...
      const { error } = await supabase.from('procedure_notes').insert({
        ...
        performed_by: user.id, status: 'completed', charge_amount: 0,
      });
      ...
      toast({ title: 'Failed', description: error instanceof Error ? error.message : 'Unable to save procedure.', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-3 flex items-start gap-2 text-sm"><CreditCard className="w-4 h-4 text-primary mt-0.5 shrink-0" /><p className="text-muted-foreground">Chargeable procedures are routed to Accounts before the clinical procedure can proceed. Zero-charge documentation remains available immediately.</p></div>

      <form onSubmit={submit} className="card-medical p-6 space-y-3">
        <div className="grid gap-3 md:grid-cols-3">
          <select value={pid} onChange={e => setPid(e.target.value)} className="input-medical"><option value="">Select patient…</option>{patients.map(p => <option key={p.id} value={p.id}>{p.first_name} {p.last_name}</option>)}</select>
          <select value={proc} onChange={e => applyTemplate(e.target.value)} className="input-medical">{Object.keys(PROCEDURE_TEMPLATES).map(k => <option key={k}>{k}</option>)}</select>
          <input type="number" min={0} step="0.01" value={chargeAmount || ''} onChange={e => setChargeAmount(Number(e.target.value))} placeholder="Charge (GHS)" className="input-medical" />
        </div>
        ...
        <button className="btn-primary"><Plus className="w-4 h-4 mr-2" /> {chargeAmount > 0 ? 'Request payment approval' : 'Save procedure note'}</button>
      </form>
