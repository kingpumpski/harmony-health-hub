...
export default function ProcedureNotes() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('procedure_notes').insert({ patient_id: pid, procedure_name: proc, template_used: proc, indication, technique, findings, complications, post_op_plan: postOp, performed_by: user.id, status: 'completed', charge_amount: 0 });
