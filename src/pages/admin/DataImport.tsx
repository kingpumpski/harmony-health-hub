...
export default function DataImport() {
  ...
  const validate = (row: Row, index: number) => { for (const field of schemas[entity as Exclude<Entity, 'diagnosis_workbook' | 'stg_ghana'>].required) if (!row[field]) return `Row ${index + 2}: missing ${field}`; if (entity === 'patients' && row.email && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(String(row.email))) return `Row ${index + 2}: invalid email`; return null; };
  ...
  return <div className="space-y-6 animate-fade-in">
    ...
    <div className="rounded-xl border border-warning/20 bg-warning/5 p-3 text-sm text-muted-foreground">Review the preview before importing. STG-Ghana is a controlled clinical reference dataset: upload and validate the approved Ghana STG workbook before clinicians can use its diagnoses.</div>
    <div className="card-medical p-5 space-y-4">
      <select value={entity} onChange={e => { setEntity(e.target.value as Entity); setRows([]); setErrors([]); setAssessment([]); }} className="input-medical w-full">
        <option value="patients">Patients</option><option value="pharmacy_inventory">Pharmacy inventory</option><option value="icd_codes">ICD-10 codes</option><option value="stg_ghana">STG-Ghana — diagnosis dataset (required)</option><option value="diagnosis_workbook">Diagnosis workbook — assess only</option>
      </select>
      {entity === 'stg_ghana' && <div className="rounded-lg border border-border bg-muted/20 p-3 text-sm"><div className="font-medium">Ghana Standard Treatment Guidelines</div><div className="text-xs text-muted-foreground mt-1">Dataset target: GH-STG · 7th Edition (2017). Current loaded diagnosis records: {stgDatasetRows ?? 'checking…'}.</div>{stgDatasetRows === 0 && <div className="text-xs text-warning mt-1">No STG-Ghana diagnosis dataset is loaded yet. Select the approved Excel workbook to assess it first; assessment does not change clinical data.</div>}</div>}
      <input type="file" accept={entity === 'stg_ghana' || entity === 'diagnosis_workbook' ? '.xlsx,.xls' : '.csv,.xlsx,.xls'} onChange={e => { const file = e.target.files?.[0]; if (!file) return; entity === 'stg_ghana' || entity === 'diagnosis_workbook' ? assessWorkbook(file) : parse(file); }} className="input-medical w-full" />
