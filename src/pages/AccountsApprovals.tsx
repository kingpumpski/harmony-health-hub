...
import { BadgeCheck, Banknote, ShieldCheck, XCircle, Settings2, Clock3, Activity, CheckCircle2, LogOut } from 'lucide-react';
...
export default function AccountsApprovals() {
  ...
  const [dischargeNotices, setDischargeNotices] = useState<Array<{ id: string; message: string; created_at: string }>>([]);
  ...
  const load = useCallback(async () => {
    const [filtered, summary, notices] = await Promise.all([
      ...
      supabase.from('notifications').select('id,message,created_at').eq('recipient_role', 'accountant').eq('category', 'payment').eq('title', 'Discharged patient ready for billing reconciliation').eq('is_read', false).order('created_at', { ascending: false }).limit(50),
    ]);
    ...
    setDischargeNotices((notices.data ?? []) as Array<{ id: string; message: string; created_at: string }>);
