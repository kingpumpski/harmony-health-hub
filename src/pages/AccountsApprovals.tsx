...
export default function AccountsApprovals() {
  ...
  return (
    <OperationalWorklistShell
      icon={Banknote}
      eyebrow="Business & Reporting · Accounts"
      title="Accounts Approvals"
      description="Review payment-gated services, release eligible work, manage authorised exceptions and reconcile discharged inpatient accounts."
      actions={(
        <select aria-label="Accounts workflow status filter" value={filter} onChange={(e) => setFilter(e.target.value as ServiceOrderStatus)} className="input-medical min-w-[220px]">
          <option value="pending_payment_approval">Awaiting payment approval</option><option value="released">Released</option><option value="in_progress">In progress</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option>
        </select>
      )}
      counters={counterCards.map(({ label, value, tone, surface }) => ({ label, value, tone, surface }))}
      beforeList={(
        <>
          <section className={`card-medical p-5 ${counts.discharged > 0 ? 'ring-1 ring-warning/30' : ''}`} aria-labelledby="discharge-reconciliation-heading">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div><h2 id="discharge-reconciliation-heading" className="font-semibold flex items-center gap-2"><LogOut className="w-4 h-4 text-warning" aria-hidden="true" /> Discharge reconciliation queue <span className="rounded-full bg-warning/10 px-2 py-0.5 text-warning text-xs">{counts.discharged}</span></h2><p className="text-xs text-muted-foreground">Discharged inpatient episodes awaiting Accounts review of the complete patient bill.</p></div>
              <button type="button" onClick={() => window.location.assign('/billing')} className="btn-secondary">Open Billing</button>
            </div>
            {dischargeNotices.length > 0 ? <div className="mt-4 space-y-2">{dischargeNotices.map((notice) => { const result = reconciliations[notice.id]; return <article key={notice.id} className="rounded-xl border border-warning/20 bg-warning/5 p-3 flex flex-col gap-3"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm">{notice.message}</p><p className="text-[11px] text-muted-foreground">{new Date(notice.created_at).toLocaleString()}</p></div><div className="flex flex-wrap gap-2"><button type="button" disabled={busyId === notice.id} onClick={() => void reconcileDischarge(notice)} className="btn-primary text-xs disabled:opacity-50"><ReceiptText className="w-3 h-3 inline mr-1" aria-hidden="true" />{busyId === notice.id ? 'Reconciling…' : 'Reconcile final bill'}</button><button type="button" onClick={() => openBilling(notice.related_patient_id)} className="btn-secondary text-xs">Open patient billing</button><button type="button" onClick={() => void acknowledgeDischarge(notice.id)} className="btn-ghost text-xs">Acknowledge</button></div></div>{result && <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs"><div><span className="text-muted-foreground">Final bill</span><p className="font-semibold">{money(result.gross)}</p></div><div><span className="text-muted-foreground">Paid</span><p className="font-semibold">{money(result.paid)}</p></div><div><span className="text-muted-foreground">Outstanding</span><p className="font-semibold">{money(result.outstanding)}</p></div><div><span className="text-muted-foreground">Insurance claimed</span><p className="font-semibold">{money(result.insuranceClaimed)}</p></div><div><span className="text-muted-foreground">State</span><p className="font-semibold capitalize">{result.status.replaceAll('_', ' ')}</p></div></div>}</article>; })}</div> : <p className="mt-4 text-sm text-muted-foreground">No discharged inpatient accounts are currently awaiting reconciliation.</p>}
          </section>
          {isAdmin && <section className="card-medical p-5" aria-labelledby="routing-style-heading"><div className="flex items-center gap-2 mb-2 font-semibold"><Settings2 className="w-4 h-4" aria-hidden="true" /><h2 id="routing-style-heading">Patient routing style</h2></div><div className="flex flex-wrap gap-4 text-sm"><label className="flex items-center gap-2"><input type="radio" checked={flow === 'strict'} onChange={() => void changeFlow('strict')} /> Accounts stop before every step</label><label className="flex items-center gap-2"><input type="radio" checked={flow === 'streamlined'} onChange={() => void changeFlow('streamlined')} /> Streamlined routing between clinical steps</label></div></section>}
        </>
      )}
      listTitle="Payment approval worklist"
      listDescription="Release paid services to departments, activate eligible daily coverage, or apply an authorised Accounts override."
      listMeta={`${orders.length} record${orders.length === 1 ? '' : 's'} shown · ${STATUS_LABEL[filter]}`}
      loading={false}
      empty={orders.length === 0}
      emptyTitle="No accounts records"
      emptyDescription="No service orders match the selected Accounts status."
    >
      <div className="divide-y divide-border">
        {orders.map((order) => { const patient = order.patients; const busy = busyId === order.id; return <article key={order.id} className="p-4 sm:p-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{order.service_name}</h2><span className="status-badge">{STATUS_LABEL[order.status]}</span></div><p className="text-sm text-muted-foreground mt-1">{patient?.first_name} {patient?.last_name} · {patient?.patient_code} · {order.department}</p>{patient?.insurance_provider && <p className="text-xs text-primary mt-1">Insurance: {patient.insurance_provider} · {patient.insurance_number ?? 'no number on file'}</p>}</div><div className="flex flex-col sm:flex-row sm:items-center gap-3 shrink-0"><span className="text-lg font-semibold tabular-nums">GHS {Number(order.amount).toFixed(2)}</span>{order.status === 'pending_payment_approval' && <div className="flex flex-wrap gap-2">{patient?.insurance_provider && <button type="button" disabled={busy} onClick={() => void activateCoverage(order)} className="btn-secondary inline-flex items-center gap-2 disabled:opacity-50"><ShieldCheck className="w-4 h-4" aria-hidden="true" /> Activate today's coverage</button>}<button type="button" disabled={busy} onClick={() => void approve(order)} className="btn-primary inline-flex items-center gap-2 disabled:opacity-50"><BadgeCheck className="w-4 h-4" aria-hidden="true" /> Approve & release</button><button type="button" disabled={busy} onClick={() => void override(order)} className="btn-secondary inline-flex items-center gap-2 disabled:opacity-50"><ShieldCheck className="w-4 h-4" aria-hidden="true" /> Override</button><button type="button" disabled={busy} onClick={() => void reject(order)} className="btn-ghost inline-flex items-center gap-2 disabled:opacity-50"><XCircle className="w-4 h-4" aria-hidden="true" /> Cancel</button></div>}</div></article>; })}
      </div>
    </OperationalWorklistShell>
  );
}
