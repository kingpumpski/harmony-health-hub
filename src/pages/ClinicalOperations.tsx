...
export default function ClinicalOperations() {
  ...
  const save = async () => {
    if (!user || busy) return;
    if (tab === 'capacity' && (!form.name?.trim() || !form.code?.trim())) { toast({ title: 'Complete required fields', description: 'Ward name and ward code are required.', variant: 'destructive' }); return; }
    if (tab !== 'capacity' && !patientId) { toast({ title: 'Select a patient', description: 'A patient must be selected before creating this operational record.', variant: 'destructive' }); return; }
    const requiredByTab: Partial<Record<Tab, string[]>> = { nursing: ['problem', 'goal', 'interventions'], emergency: ['complaint'], theatre: ['procedure_name'], transfusion: ['blood_product', 'unit_identifier'], insurance: ['payer_name', 'amount_claimed'] };
    const missing = (requiredByTab[tab] ?? []).filter((key) => !form[key]?.trim());
    if (missing.length) { toast({ title: 'Complete required fields', description: 'Please complete all required fields before saving.', variant: 'destructive' }); return; }
    setBusy(true);
    try {
