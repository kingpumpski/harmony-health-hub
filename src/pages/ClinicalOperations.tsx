...
export default function ClinicalOperations() {
  ...
  const transition = async (id: string, status: string) => {
    ...
    const current = String(rows.find((row) => String(row.id) === id)?.status ?? '');
    if (!current || !canTransitionWorkflow(tab as 'emergency' | 'theatre' | 'transfusion' | 'insurance', current, nextStatus)) {
      toast({ title: 'Invalid workflow transition', description: 'Choose the next permitted status for the current record.', variant: 'destructive' });
      return;
    }
    setBusy(true);
    try {
      let error: null | { message: string } = null;
      if (tab === 'emergency') error = (await supabase.rpc('transition_emergency_case', { _case_id: id, _status: nextStatus, _disposition: nextStatus === 'discharged' ? 'Discharged from emergency' : nextStatus === 'referred' ? 'Referred for further care' : nextStatus === 'left_without_being_seen' ? 'Left without being seen' : null } as never)).error;
      else if (tab === 'theatre') error = (await supabase.rpc('transition_theatre_case', { _case_id: id, _status: nextStatus, _cancellation_reason: nextStatus === 'cancelled' || nextStatus === 'postponed' ? 'Status changed from clinical operations' : null } as never)).error;
      else if (tab === 'transfusion') error = (await supabase.rpc('record_transfusion_event', { _record_id: id, _status: nextStatus, _reaction_observed: false, _reaction_notes: null } as never)).error;
      else if (tab === 'insurance') error = (await supabase.rpc('transition_insurance_claim_canonical', { _claim_id: id, _status: nextStatus, _amount_approved: null, _amount_paid: null, _rejection_reason: nextStatus === 'rejected' ? 'Rejected in claims workflow' : null, _notes: status === 'rejected' ? 'Rejected in claims workflow' : 'Status transition from claims queue' } as never)).error;
      if (error) throw error;
