...
const tabs: [Tab, string, ElementType][] = [
  ['capacity', 'Ward & Beds', BedDouble],
  ['nursing', 'Nursing', ClipboardList],
  ['emergency', 'Emergency', Siren],
  ['theatre', 'Theatre', Stethoscope],
  ['transfusion', 'Transfusion', Droplets],
...
export default function ClinicalOperations() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (tab === 'capacity') {
        ...
        if (tab === 'nursing') {
          const result = await supabase.rpc('create_nursing_care_plan', { _patient_id: patientId, _problem: form.problem, _goal: form.goal, _interventions: form.interventions, _priority: form.priority || 'routine' } as never);
          error = result.error;
        } else if (tab === 'emergency') {
          const result = await supabase.rpc('create_emergency_case', { _patient_id: patientId, _chief_complaint: form.complaint, _acuity: form.acuity || 'urgent', _arrival_mode: form.arrival_mode || 'walk_in', _assigned_officer: user.id } as never);
          error = result.error;
        } else if (tab === 'theatre') {
          const result = await supabase.rpc('create_theatre_case', { _patient_id: patientId, _procedure_name: form.procedure_name, _scheduled_start: form.scheduled_start || null, _theatre_name: form.theatre_name || null, _urgency: form.urgency || 'elective', _surgeon_id: user.id } as never);
          error = result.error;
        } else if (tab === 'transfusion') {
          const result = await supabase.rpc('create_transfusion_record', { _patient_id: patientId, _blood_product: form.blood_product, _unit_identifier: form.unit_identifier, _blood_group: form.blood_group || null, _consent_confirmed: form.consent === 'true' } as never);
          error = result.error;
        } else {
          const result = await supabase.rpc('create_insurance_claim_draft', { _patient_id: patientId, _payer_name: form.payer_name, _member_number: form.member_number || null, _amount_claimed: Number(form.amount_claimed || 0), _invoice_id: null } as never);
          error = result.error;
        }
      }
      ...
      toast({ title: 'Save failed', description: error instanceof Error ? error.message : 'Please review the form.', variant: 'destructive' });
    } finally {
      setBusy(false);
    }
  };
  ...
  const transition = async (id: string, status: string) => {
    ...
    try {
      let error: null | { message: string } = null;
      if (tab === 'emergency') {
        const result = await supabase.rpc('transition_emergency_case', { _case_id: id, _status: status, _disposition: status === 'discharged' ? 'Discharged from emergency' : status === 'referred' ? 'Referred for further care' : status === 'left_without_being_seen' ? 'Left without being seen' : null } as never);
        error = result.error;
      } else if (tab === 'theatre') {
        const result = await supabase.rpc('transition_theatre_case', { _case_id: id, _status: status, _cancellation_reason: status === 'cancelled' || status === 'postponed' ? 'Status changed from clinical operations' : null } as never);
        error = result.error;
      } else if (tab === 'transfusion') {
        const result = await supabase.rpc('record_transfusion_event', { _record_id: id, _status: status, _reaction_observed: false, _reaction_notes: null } as never);
        error = result.error;
      } else if (tab === 'insurance') {
        const result = await supabase.rpc('transition_insurance_claim', { _claim_id: id, _to_status: status, _notes: status === 'rejected' ? 'Rejected in claims workflow' : 'Status transition from claims queue' } as never);
        error = result.error;
      }
      if (error) throw error;
      ...
      toast({ title: 'Transition failed', description: error instanceof Error ? error.message : 'Workflow transition failed.', variant: 'destructive' });
    } finally {
      setBusy(false);
    }
  };
