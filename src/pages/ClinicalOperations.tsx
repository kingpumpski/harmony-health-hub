...
<button onClick={() => void load()} className="btn-secondary inline-flex items-center gap-2 self-start" aria-label="Refresh clinical operations" disabled={busy}><RefreshCw className={`h-4 w-4 ${busy ? 'animate-spin' : ''}`} /> Refresh</button>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-6">{tabs.filter(([id]) => allowedTabs.includes(id)).map(([id, labelText, Icon]) => <button key={id} onClick={() => { setTab(id); setForm({}); setPatientId(''); }} className={`min-w-0 rounded-xl border p-3 text-left text-sm transition-colors ${tab === id ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-card hover:bg-muted'}`}><Icon className="mb-2 h-5 w-5" /><span className="block truncate">{labelText}</span></button>)}</div>
      <div className="space-y-4 rounded-2xl border bg-card p-5 shadow-sm" aria-live="polite">
