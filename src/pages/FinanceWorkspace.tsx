...
export default function FinanceWorkspace() {
  return <div className="space-y-6 animate-fade-in"><header><div className="flex items-center gap-3"><CreditCard className="h-7 w-7 text-primary" /><div><h1 className="text-2xl font-heading font-bold">Finance</h1><p className="text-sm text-muted-foreground">The financial lifecycle around patient care, billing, approvals, insurance and reconciliation.</p></div></div></header><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{items.map(item => { const Icon=item.icon; return <Link key={item.href} to={item.href} className="card-medical p-5 transition-transform hover:-translate-y-0.5"><Icon className="h-5 w-5 text-primary" /><h2 className="mt-4 font-semibold">{item.label}</h2><p className="mt-1 text-sm text-muted-foreground">{item.description}</p></Link>; })}</div></div>;
}
