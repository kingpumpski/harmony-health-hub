...
export default function Appointments() {
  ...
  const load = async () => {
    const [{ data: pts, error: patientError }, { data: aps, error: appointmentError }] = await Promise.all([
      ...
      supabase.from('appointments').select('*').order('scheduled_at', { ascending: false }).limit(100),
    ]);
  ...
  const create = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.from('appointments').insert({
      patient_id: pid, scheduled_at: new Date(when).toISOString(), reason: reason || null,
      department: dept, status: 'scheduled', treatment_status: 'scheduled',
    }).select().single();
    ...
    await notifyRoles(['practitioner', 'nurse', 'midwife', 'specialist_nurse', 'front_desk'], {
      title: 'New appointment', message: `${name} scheduled for ${dept} on ${new Date(when).toLocaleString()}`,
      severity: 'info', category: 'appointment', link: '/appointments', relatedPatientId: pid, relatedEntityId: data.id,
    });
  ...
  const claim = async (appointmentId: string) => {
    ...
    toast({ title: 'Appointment assigned to you', description: 'You can now start treatment and update the care status.' });
    await load();
  };
  ...
  const saveEdit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await supabase.rpc('update_appointment_workflow' as never, {
      _appointment_id: editing.id,
      _scheduled_at: new Date(String(form.get('scheduled_at'))).toISOString(),
      _department: String(form.get('department') ?? ''),
      _reason: String(form.get('reason') ?? ''),
      _treatment_status: String(form.get('treatment_status') ?? 'scheduled'),
      _treatment_notes: String(form.get('treatment_notes') ?? ''),
    } as never);
    setSaving(false);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-heading font-bold flex items-center gap-2"><Calendar className="w-6 h-6 text-primary" /> Appointments</h1>
        <p className="text-muted-foreground">Schedule visits, assign attending officers and progress the patient through treatment.</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        {isStaff && <form onSubmit={create} className="card-medical p-5 space-y-3 h-fit">
          ...
          <input required type="datetime-local" value={when} onChange={(e) => setWhen(e.target.value)} className="input-medical w-full" />
          <select value={dept} onChange={(e) => setDept(e.target.value)} className="input-medical w-full"><option>General Outpatient</option><option>Specialist Consultation</option><option>Maternity</option><option>Fertility</option><option>Telemedicine</option></select>
        ...
        <div className="card-medical p-5">
          <div className="flex items-center justify-between mb-3"><h2 className="font-semibold">Upcoming & Recent</h2><span className="text-xs text-muted-foreground">{appts.length} records</span></div>
            ...
            {appts.map((a) => {
              ...
              return <div key={a.id} className="rounded-xl border border-border p-4 space-y-3">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  ...
                  <span className="text-xs px-2 py-1 rounded-full bg-muted text-muted-foreground capitalize shrink-0">{treatmentStatus.replaceAll('_', ' ')}</span>
                </div>
      ...
      {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true">
        <form onSubmit={saveEdit} className="w-full max-w-xl rounded-2xl bg-card border border-border shadow-xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold">Edit appointment</h2><p className="text-sm text-muted-foreground">{patientName.get(editing.patient_id) || 'Patient'}</p></div><button type="button" onClick={() => setEditing(null)} className="btn-secondary">Close</button></div>
