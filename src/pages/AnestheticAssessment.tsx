...
export default function AnestheticAssessment() {
  ...
  const submit = async (e: React.FormEvent) => { e.preventDefault(); if (!pid || !user?.id) return; setSaving(true); const { error } = await supabase.from('anesthetic_assessments').insert({ patient_id: pid, asa_class: asa, airway_assessment: airway, cardiovascular: cv, respiratory: resp, allergies, medications: meds, fasting_status: fasting, conclusions: conclusion, cleared_for_procedure: cleared, cleared_by: cleared ? user.id : null, status: 'completed' }); setSaving(false); if (error) return toast({ title: 'Failed', description: error.message, variant: 'destructive' }); toast({ title: cleared ? 'Assessment saved — clearance recorded' : 'Assessment saved' }); reset(); void load(); };
