...
export default function TheatreBoard() {
  ...
  const createCase = async (event: FormEvent) => {
    ...
    if (!form.patientId || !form.procedureName.trim() || !form.scheduledStart) {
      toast({ title: 'Patient, procedure and scheduled start are required', variant: 'destructive' });
      return;
    }
    setBusy(true);
    const { error } = await (supabase as any).rpc('create_theatre_case', {
      _patient_id: form.patientId,
      _procedure_name: form.procedureName.trim(),
      _scheduled_start: new Date(form.scheduledStart).toISOString(),
      _theatre_name: form.theatreName.trim() || null,
      _urgency: form.urgency,
      _surgeon_id: null,
    });
    setBusy(false);
    if (error) {
      toast({ title: 'Theatre case creation failed', description: error.message, variant: 'destructive' });
      return;
    }
    setForm({ patientId: '', procedureName: '', scheduledStart: '', theatreName: '', urgency: 'elective' });
    toast({ title: 'Theatre case registered' });
    void load();
  };

  const transition = async (id: string, status: string) => {
    setBusy(true);
    const { error } = await (supabase as any).rpc('transition_theatre_case', { _case_id: id, _status: status, _cancellation_reason: status === 'cancelled' || status === 'postponed' ? 'Status changed from theatre board' : null });
    setBusy(false);
    if (error) toast({ title: 'Transition failed', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Theatre status updated' }); void load(); }
  };

  const assignAnesthetist = async (id: string, officerId: string) => {
    if (!officerId) return;
    setBusy(true);
    const { error } = await (supabase as any).rpc('assign_theatre_anesthetist', { _case_id: id, _anesthetist_id: officerId });
    setBusy(false);
    if (error) toast({ title: 'Assignment failed', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Anesthetist assigned' }); void load(); }
  };
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div><h1 className="text-2xl font-heading font-bold flex items-center gap-2"><CalendarClock className="h-6 w-6" />Theatre Board</h1><p className="text-sm text-muted-foreground">Theatre intake, scheduling, anesthetist assignment and controlled clinical transitions.</p></div>
