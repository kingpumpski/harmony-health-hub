...
export default function TransfusionBoard() {
  ...
  const create = async (event: FormEvent) => {
    ...
    if (!form.patientId || !form.bloodProduct.trim() || !form.unitIdentifier.trim()) {
      toast({ title: 'Patient, blood product and unit identifier are required', variant: 'destructive' });
      return;
    }
    if (!form.consentConfirmed) {
      toast({ title: 'Confirm documented transfusion consent before scheduling', variant: 'destructive' });
      return;
    }
    setBusy(true);
    const { error } = await (supabase as any).rpc('create_transfusion_record', {
      _patient_id: form.patientId,
      _blood_product: form.bloodProduct.trim(),
      _unit_identifier: form.unitIdentifier.trim(),
      _blood_group: form.bloodGroup.trim() || null,
    ...
    setForm({ patientId: '', bloodProduct: '', unitIdentifier: '', bloodGroup: '', consentConfirmed: false });
  ...
  const transition = async (id: string, status: string) => {
    setBusy(true);
    const { error } = await (supabase as any).rpc('record_transfusion_event', { _record_id: id, _status: status, _reaction_observed: false, _reaction_notes: null });
    setBusy(false);
    if (error) toast({ title: 'Transition failed', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Transfusion status updated' }); void load(); }
  };

  const reaction = async (id: string) => {
    const notes = window.prompt('Document the observed reaction.');
    if (notes === null) return;
    setBusy(true);
    const { error } = await (supabase as any).rpc('record_transfusion_reaction', { _record_id: id, _reaction_observed: true, _reaction_notes: notes.trim() || null });
    setBusy(false);
    if (error) toast({ title: 'Reaction update failed', description: error.message, variant: 'destructive' });
    else { toast({ title: 'Reaction documented' }); void load(); }
  };
