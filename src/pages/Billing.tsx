...
export default function Billing() {
  ...
  return (
    <OperationalWorklistShell
      ...
      description="Prepare and reconcile one patient account across consultations, diagnostics, pharmacy, procedures, accommodation and other services."
      actions={<button type="button" onClick={() => { playWorkflowSound('info'); void loadBillable(); }} className="btn-secondary inline-flex items-center gap-2" disabled={!patientId || loading}><RefreshCw className="w-4 h-4" aria-hidden="true" />Refresh account</button>}
      counters={patientId ? counters.map(({ label, value, tone, surface }) => ({ label, value, tone, surface })) : []}
      beforeList={<section className="card-medical p-5 space-y-4" aria-labelledby="billing-account-selector"><div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_170px_170px_auto] items-end"><label className="text-sm space-y-1"><span id="billing-account-selector" className="font-medium">Patient</span><select aria-label="Select patient account" value={patientId} onChange={(e) => setPatientId(e.target.value)} className="input-medical w-full"><option value="">Select patient…</option>{patients.map((p) => <option key={p.id} value={p.id}>{p.first_name} {p.last_name} · {p.patient_code}</option>)}</select></label><label className="text-sm space-y-1"><span>From</span><input aria-label="Billing period start" type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="input-medical w-full" /></label><label className="text-sm space-y-1"><span>To</span><input aria-label="Billing period end" type="date" value={to} onChange={(e) => setTo(e.target.value)} className="input-medical w-full" /></label><button type="button" onClick={() => void loadBillable()} className="btn-primary inline-flex justify-center gap-2" disabled={!patientId || loading}>{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <CalendarDays className="w-4 h-4" />}Prepare bill</button></div></section>}
      listTitle={patientId ? "Final bill ledger" : "Patient account worklist"}
      listDescription={patientId ? "Recorded services, payment history and release state for the selected billing period." : "Select a patient to prepare and reconcile their account."}
      listMeta={patientId ? items.length + " records · " + selected.length + " selected" : "Patient selection required"}
      loading={loading}
      empty={Boolean(patientId) && !loading && items.length === 0}
      emptyTitle="No billable items"
      emptyDescription="No billable services were found for this patient and period."
    >
