...
export default function Billing() {
  ...
  const addWalkIn = async () => {
    ...
    playWorkflowSound('success'); toast.success('Walk-in service added to the patient account.'); setWalkInCode(''); setWalkInQty(1); await loadBillable();
  };
  ...
  return <div className="space-y-6 animate-fade-in">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-heading font-bold flex items-center gap-2"><CreditCard className="w-6 h-6 text-primary" />Billing & Patient Account</h1><p className="text-muted-foreground">One patient account across consultations, diagnostics, drugs, procedures, accommodation and other services.</p></div><button onClick={() => { playWorkflowSound('info'); void loadBillable(); }} className="btn-secondary inline-flex items-center gap-2"><RefreshCw className="w-4 h-4" />Refresh account</button></header>

    {patientId && <section aria-label="Patient financial summary" className="grid grid-cols-2 lg:grid-cols-4 gap-3">{counters.map(({ label, value, icon: Icon, tone, surface }) => <div key={label} className={`card-medical ${surface} p-4 transition-all duration-300 hover:-translate-y-1`}><div className="flex items-start justify-between"><div><p className="text-xs text-muted-foreground">{label}</p><p className={`text-xl font-bold ${tone}`}>{value}</p></div><Icon className={`w-5 h-5 ${tone}`} /></div></div>)}</section>}
    ...
    {patientId && <section className="card-medical p-5"><div className="flex items-center gap-2 mb-3"><Plus className="w-4 h-4 text-primary" /><div><h2 className="font-semibold">Walk-in service</h2><p className="text-xs text-muted-foreground">Add an additional service to the same patient account.</p></div></div><div className="grid md:grid-cols-[1fr_120px_auto] gap-2 items-end"><select value={walkInCode} onChange={(e) => setWalkInCode(e.target.value)} className="input-medical"><option value="">Select service tariff…</option>{tariffs.map((t) => <option key={t.id} value={t.service_code}>{t.service_name} · {t.department} · {money(Number(t.amount))}/{t.unit}</option>)}</select><input type="number" min={1} value={walkInQty} onChange={(e) => setWalkInQty(Number(e.target.value))} className="input-medical" /><button onClick={() => void addWalkIn()} className="btn-primary">Add to bill</button></div></section>}
