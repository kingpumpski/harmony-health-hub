...
interface ServiceOrderRpcRow { id: string; department: string; service_name: string; patient_id: string; status: ServiceOrderStatus; }
...
export interface CreateServiceOrderInput {
  patientId: string; encounterId?: string | null; department: ServiceDepartment; serviceName: string;
  amount?: number; relatedEntityId?: string | null; notes?: string | null; requestedBy?: string | null;
  invoiceId?: string | null; invoiceItemId?: string | null;
  orderType?: 'lab' | 'imaging' | 'procedure' | 'drug' | null; serviceCode?: string | null;
}

export async function getPaymentFlow(): Promise<PaymentFlow> {
  const { data, error } = await supabase.from('facility_settings').select('payment_flow').eq('id', 'default').maybeSingle();
  if (error) throw error;
  return data?.payment_flow === 'strict' ? 'strict' : 'streamlined';
}

export async function setPaymentFlow(flow: PaymentFlow) {
  const { error } = await workflowRpc.rpc('set_facility_routing_mode', { _mode: flow === 'strict' ? 'pay_before_each_step' : 'streamlined' });
...
export async function createServiceOrder(input: CreateServiceOrderInput) {
  ...
  if (error) throw new Error(error.message);
  const dataRow = data as ServiceOrderRpcRow;
  await notifyRoles(['accountant', 'front_desk'], {
    ...
    severity: 'warning', category: 'payment', link: '/accounts-approvals',
    relatedPatientId: input.patientId, relatedEntityId: dataRow.id,
  });
  return dataRow;
}
...
export async function grantServiceOrderOverride(orderId: string, reason: string) {
  const { data, error } = await workflowRpc.rpc('grant_service_order_override', { _service_order_id: orderId, _reason: reason });
  if (error) throw new Error(error.message); return data;
}

export async function cancelServiceOrder(orderId: string, reason = 'Cancelled by authorised staff') {
  const { data, error } = await workflowRpc.rpc('cancel_service_order', { _service_order_id: orderId, _reason: reason });
  if (error) throw new Error(error.message); return data;
}

export async function markServiceOrderInProgress(orderId: string) {
  const { data, error } = await workflowRpc.rpc('mark_service_order_in_progress', { _service_order_id: orderId });
  if (error) throw new Error(error.message); return data;
}

export async function completeServiceOrder(orderId: string) {
  const { data, error } = await workflowRpc.rpc('complete_service_order', { _service_order_id: orderId });
  if (error) throw new Error(error.message); return data;
}

export async function isReleased(relatedEntityId: string) {
  const { data, error } = await supabase.from('service_orders').select('status').eq('related_entity_id', relatedEntityId).maybeSingle();
  if (error) throw error;
  return Boolean(data && (data.status === 'released' || data.status === 'in_progress' || data.status === 'completed'));
}

export const STATUS_LABEL: Record<ServiceOrderStatus, string> = {
  pending_payment_approval: 'Awaiting payment approval', released: 'Released to department',
