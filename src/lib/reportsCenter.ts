...
export interface ReportDefinition {
  ...
  is_active: boolean;
  implementation_status: 'seeded' | 'mapped' | 'validated' | 'retired';
...
export interface ReportSnapshot {
  total: number;
  by_dimension: Array<{ dimension: string; value: string; count: number }>;
  source: string;
  warning?: string;
}
...
const SOURCE_TABLES: Record<string, string> = {
  opd_morbidity: 'encounters + diagnoses',
  opd_attendance: 'appointments',
  laboratory: 'lab_orders + lab_results',
  inpatient_days: 'admissions / inpatient module',
  surgeries: 'procedure/theatre module',
  maternity: 'maternity module',
};
...
async function countTable(table: 'appointments' | 'encounters' | 'diagnoses' | 'lab_orders' | 'lab_results', dateColumn: string, start: string, end: string) {
  const { count, error } = await supabase.from(table).select('*', { count: 'exact', head: true }).gte(dateColumn, start).lte(dateColumn, end);
...
export async function listFacilities(): Promise<HealthcareFacility[]> {
  const { data, error } = await supabase.from('healthcare_facilities').select('id,name,facility_code,facility_type,district,region,dhims2_uid,is_active').eq('is_active', true).order('name');
  if (error) throw error;
...
export async function createFacility(input: Omit<HealthcareFacility, 'id' | 'is_active'>): Promise<HealthcareFacility> {
  const { data, error } = await supabase.from('healthcare_facilities').insert(input).select('id,name,facility_code,facility_type,district,region,dhims2_uid,is_active').single();
  if (error) throw error;
  const facility = data as HealthcareFacility;
  const { error: seedError } = await supabase.rpc('seed_facility_reports', { _facility_id: facility.id });
  if (seedError) throw seedError;
  return facility;
}

export async function listDefinitions(): Promise<ReportDefinition[]> {
  const { data, error } = await supabase.from('report_definitions').select('*, report_categories(*)').eq('is_active', true).order('report_code');
  if (error) throw error;
  return (data ?? []).map((row) => ({ ...row, category: Array.isArray(row.report_categories) ? row.report_categories[0] ?? null : row.report_categories, parameters: Array.isArray(row.parameters) ? row.parameters : [] })) as ReportDefinition[];
}

export async function listFacilityConfigs(facilityId: string): Promise<FacilityReportConfig[]> {
  const { data, error } = await supabase.from('facility_report_config').select('*, report_definitions(*, report_categories(*))').eq('facility_id', facilityId).order('report_id');
  if (error) throw error;
  return (data ?? []).map((row) => ({ ...row, report: row.report_definitions })) as FacilityReportConfig[];
}

export async function setReportEnabled(facilityId: string, reportId: string, enabled: boolean) {
  const { error } = await supabase.from('facility_report_config').upsert({ facility_id: facilityId, report_id: reportId, is_enabled: enabled }, { onConflict: 'facility_id,report_id' });
  if (error) throw error;
}
...
export async function generateRun(facilityId: string, period: string, configs: FacilityReportConfig[]) {
  ...
  const { data: run, error: runError } = await supabase.from('report_generation_runs').insert({
    facility_id: facilityId,
    period_start: start.slice(0, 10),
    period_end: end.slice(0, 10),
    frequency: 'monthly',
    status: 'processing',
    total_reports: enabled.length,
  ...
  const itemRows = enabled.map((config) => ({ run_id: run.id, report_id: config.report_id, status: 'processing', output_format: 'xlsx' }));
  const { data: items, error: itemError } = await supabase.from('report_generation_items').insert(itemRows).select('*');
  ...
  for (const item of items ?? []) {
    ...
    await supabase.from('report_generation_items').update({ status, data_snapshot: snapshot, validation_messages: snapshot.warning ? [snapshot.warning] : [], file_name: `${config.report.report_code}_${period}.xlsx`, completed_at: new Date().toISOString() }).eq('id', item.id);
  }
  failed = enabled.length - success - warning;
  const finalStatus = failed > 0 ? 'partial_failed' : warning > 0 ? 'completed' : 'completed';
  const { data: finalRun, error: finalError } = await supabase.from('report_generation_runs').update({ status: finalStatus, success_count: success, warning_count: warning, failed_count: failed, completed_at: new Date().toISOString() }).eq('id', run.id).select('*').single();
  if (finalError) throw finalError;
  return finalRun as ReportRun;
}

export async function getRunItems(runId: string): Promise<ReportRunItem[]> {
  const { data, error } = await supabase.from('report_generation_items').select('*').eq('run_id', runId).order('created_at');
  if (error) throw error;
...
export async function downloadRunWorkbook(run: ReportRun, items: ReportRunItem[], facility: HealthcareFacility) {
  ...
  const filename = `Monthly_Reports_${facility.name.replace(/[^A-Za-z0-9]+/g, '_')}_${run.period_start.slice(0, 7)}.xlsx`;
...
export function downloadRunManifestCsv(run: ReportRun, items: ReportRunItem[], facility: HealthcareFacility) {
  ...
  anchor.href = url; anchor.download = `Reports_Manifest_${facility.name.replace(/[^A-Za-z0-9]+/g, '_')}_${run.period_start.slice(0, 7)}.csv`; anchor.click(); URL.revokeObjectURL(url);
}
