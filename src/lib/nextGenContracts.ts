...
export interface AIClinicalOutput<T = unknown> {
  ...
  output: T;
  generatedAt: string;
  provenance?: { sourceSystem?: string; sourceRecordIds?: string[]; correlationId?: string; auditRequired?: boolean };
}
...
export function validateEnvelope(value: unknown): value is InteroperabilityEnvelope {
  ...
  if ('destinationSystem' in candidate && candidate.destinationSystem !== undefined && (typeof candidate.destinationSystem !== 'string' || candidate.destinationSystem.trim() === '')) return false;
  ...
  if ('patientReference' in candidate && candidate.patientReference !== undefined && (typeof candidate.patientReference !== 'string' || candidate.patientReference.trim() === '')) return false;
