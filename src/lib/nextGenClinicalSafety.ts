...
export function assertClinicalActionSafe(action: string, context: { authorised: boolean; confirmed: boolean; audited: boolean; online: boolean }): void {
  ...
  if (guard.requiresAudit && !context.audited) throw new Error(`Audit record required for: ${action}`);
