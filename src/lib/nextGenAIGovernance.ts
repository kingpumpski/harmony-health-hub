...
export function authorizeAIClinicalUse(record: AIModelGovernanceRecord, intendedUse: string): AIGovernanceDecision {
  ...
  if (!record.evaluationPassed || record.evidenceReferences.length === 0) return { allowed: false, reviewRequired: true, reason: 'Required model evaluation evidence is not passed' };
...
export function governClinicalOutput<T>(record: AIModelGovernanceRecord, output: AIClinicalOutput<T>): AIClinicalOutput<T> {
  ...
  if (!output.evidence.length) throw new Error('AI clinical output requires evidence references');
  ...
  if (output.provenance?.auditRequired && !output.provenance.correlationId) throw new Error('Audited AI output requires a correlation ID');
