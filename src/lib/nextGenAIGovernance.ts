...
export function authorizeAIClinicalUse(record: AIModelGovernanceRecord, intendedUse: string): AIGovernanceDecision {
  ...
  if (!record.evaluationPassed || !Array.isArray(record.evidenceReferences) || record.evidenceReferences.length === 0) return { allowed:false, reviewRequired:true, reason:'Required model evaluation evidence is not passed' };
  if (!intendedUse?.trim()) return { allowed:false, reviewRequired:true, reason:'AI intended use is required' };
  if (!Array.isArray(record.intendedUses) || !record.intendedUses.includes(intendedUse)) return { allowed:false, reviewRequired:true, reason:'Intended use is not registered' };
  if (Array.isArray(record.prohibitedUses) && record.prohibitedUses.includes(intendedUse)) return { allowed:false, reviewRequired:true, reason:'Intended use is explicitly prohibited' };
...
export function governClinicalOutput<T>(record: AIModelGovernanceRecord, output: AIClinicalOutput<T>): AIClinicalOutput<T> {
  ...
  if (!Array.isArray(output.evidence) || output.evidence.length === 0 || output.evidence.some((item) => !item?.source?.trim())) throw new Error('AI clinical output requires valid evidence references');
  ...
  if (Number.isNaN(Date.parse(output.generatedAt))) throw new Error('AI output generatedAt is invalid');
