...
type Card = { label: string; value: number; href: string; icon: typeof Activity; tone: string; urgent?: boolean };
...
export default function WorkflowSummary() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [appointments, reviews, services, bedsOccupied, bedsAvailable, emergency, theatre, claims, critical] = await Promise.all([
        ...
        supabase.from('service_orders').select('patient_id,department,status').in('department', ['pharmacy', 'laboratory', 'lab', 'imaging', 'radiology', 'nursing', 'maternity', 'admission', 'accounts']).in('status', ['released', 'in_progress']),
      ...
      if (!mounted) return;
      const rows = (services.data ?? []) as Array<{ patient_id: string; department: string; status: string }>;
      const countPatients = (names: string[]) => new Set(rows.filter((row) => names.includes(row.department.toLowerCase())).map((row) => row.patient_id)).size;
      ...
      const departmentAliases = [currentDepartment.toLowerCase()];
  ...
  const cards = useMemo<Card[]>(() => {
    ...
    const common: Card[] = [
      ...(appointmentRoles.includes(role) ? [
        { label: "Today's appointments", value: counts.newAppointments, href: '/appointments', icon: CalendarDays, tone: 'text-primary', urgent: counts.newAppointments > 0 },
        { label: "Today's clinical reviews", value: counts.reviews, href: '/appointments', icon: ClipboardCheck, tone: 'text-success', urgent: counts.reviews > 0 },
      ] : []),
      ...(clinicalRoles.includes(role) && counts.criticalAlerts > 0 ? [{ label: 'Critical alerts', value: counts.criticalAlerts, href: '/notifications', icon: AlertTriangle, tone: 'text-critical', urgent: true }] : []),
    ];
    if (role === 'practitioner') return [...common, { label: 'Patients waiting for you', value: counts.departmentWaiting, href: '/department-queue', icon: Users, tone: 'text-warning', urgent: counts.departmentWaiting > 0 }, { label: 'Laboratory waiting', value: counts.laboratory, href: '/laboratory', icon: FlaskConical, tone: 'text-info', urgent: counts.laboratory > 0 }, { label: 'Radiology waiting', value: counts.imaging, href: '/radiology', icon: ScanLine, tone: 'text-primary', urgent: counts.imaging > 0 }, { label: 'Pharmacy waiting', value: counts.pharmacy, href: '/pharmacy', icon: Pill, tone: 'text-success', urgent: counts.pharmacy > 0 }];
    if (role === 'lab_technician') return [...common, { label: 'Laboratory waiting', value: counts.laboratory, href: '/laboratory', icon: FlaskConical, tone: 'text-info', urgent: counts.laboratory > 0 }];
    if (role === 'pharmacist') return [...common, { label: 'Pharmacy waiting', value: counts.pharmacy, href: '/pharmacy', icon: Pill, tone: 'text-success', urgent: counts.pharmacy > 0 }, { label: 'Patients waiting', value: counts.departmentWaiting, href: '/department-queue', icon: Users, tone: 'text-warning', urgent: counts.departmentWaiting > 0 }];
    if (role === 'accountant') return [{ label: 'Payment approvals', value: counts.paymentApprovals, href: '/accounts-approvals', icon: CreditCard, tone: 'text-warning', urgent: counts.paymentApprovals > 0 }, { label: 'Claims attention', value: counts.claims, href: '/insurance-claims', icon: ShieldCheck, tone: 'text-primary', urgent: counts.claims > 0 }];
    if (role === 'nurse' || role === 'midwife' || role === 'specialist_nurse') return [...common, { label: 'Department waiting', value: counts.departmentWaiting, href: '/department-queue', icon: Users, tone: 'text-warning', urgent: counts.departmentWaiting > 0 }, { label: 'Nursing queue', value: counts.nursing, href: '/department-queue', icon: Activity, tone: 'text-info', urgent: counts.nursing > 0 }, { label: 'Admissions waiting', value: counts.admissions, href: '/admissions', icon: BedDouble, tone: 'text-primary', urgent: counts.admissions > 0 }, ...(role === 'midwife' ? [{ label: 'Maternity waiting', value: counts.maternity, href: '/maternity', icon: Baby, tone: 'text-success', urgent: counts.maternity > 0 }] : [])];
    if (role === 'front_desk') return [...common, { label: 'Department waiting', value: counts.departmentWaiting, href: '/department-queue', icon: Users, tone: 'text-warning', urgent: counts.departmentWaiting > 0 }, { label: 'Emergency queue', value: counts.emergency, href: '/emergency-board', icon: Siren, tone: 'text-critical', urgent: counts.emergency > 0 }];
    return [...common, { label: 'Laboratory waiting', value: counts.laboratory, href: '/laboratory', icon: FlaskConical, tone: 'text-info', urgent: counts.laboratory > 0 }, { label: 'Pharmacy waiting', value: counts.pharmacy, href: '/pharmacy', icon: Pill, tone: 'text-success', urgent: counts.pharmacy > 0 }, { label: 'Radiology waiting', value: counts.imaging, href: '/radiology', icon: ScanLine, tone: 'text-primary', urgent: counts.imaging > 0 }, { label: 'Accounts waiting', value: counts.paymentApprovals, href: '/accounts-approvals', icon: CreditCard, tone: 'text-warning', urgent: counts.paymentApprovals > 0 }, { label: 'Admissions waiting', value: counts.admissions, href: '/admissions', icon: BedDouble, tone: 'text-primary', urgent: counts.admissions > 0 }, { label: 'Emergency queue', value: counts.emergency, href: '/emergency-board', icon: Siren, tone: 'text-critical', urgent: counts.emergency > 0 }, { label: 'Theatre today', value: counts.theatre, href: '/theatre-board', icon: Scissors, tone: 'text-primary', urgent: counts.theatre > 0 }, { label: 'Claims attention', value: counts.claims, href: '/insurance-claims', icon: ShieldCheck, tone: 'text-warning', urgent: counts.claims > 0 }, ...(bedRoles.includes(role) ? [{ label: 'Occupied beds', value: counts.occupiedBeds, href: '/ward-bed-board', icon: Users, tone: 'text-critical', urgent: false }, { label: 'Available beds', value: counts.availableBeds, href: '/ward-bed-board', icon: BedDouble, tone: 'text-success', urgent: false }] : [])];
  }, [counts, user, department]);

  if (!user) return null;
  return <section aria-label="Live workflow queues" className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 mb-6">{cards.map(({ label, value, href, icon: Icon, tone, urgent }) => <Link key={label} to={href} className={`card-medical p-3 min-w-0 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-elevated ${urgent && value > 0 ? 'ring-1 ring-primary/20' : ''}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="text-[11px] leading-tight text-muted-foreground line-clamp-2">{label}</p><p className={`text-2xl font-bold mt-1 tabular-nums transition-all duration-300 ${urgent && value > 0 ? 'animate-pulse' : ''}`}>{value}</p><p className="text-[10px] text-muted-foreground mt-1">Open queue</p></div><Icon className={`w-5 h-5 shrink-0 ${tone}`} /></div></Link>)}</section>;
}
