...
import {
  ...
  Eye, ShieldCheck, Database, Video, Receipt, UserCog,
...
const roleNavItems: Record<string, NavItem[]> = {
  admin: [
    ...
    { icon: Pill, label: 'Pharmacy', href: '/pharmacy' }, { icon: CreditCard, label: 'Billing', href: '/billing' }, { icon: Video, label: 'Telemedicine', href: '/telemedicine' },
    { icon: Baby, label: 'Fertility', href: '/fertility' }, { icon: Utensils, label: 'Canteen', href: '/menu' }, { icon: Sparkles, label: 'AI Report', href: '/ai-report' },
    { icon: ShieldCheck, label: 'AI Hub', href: '/ai-clinical' }, { icon: UserCog, label: 'Manage Users', href: '/admin/users' }, { icon: Database, label: 'System Library', href: '/admin/system' },
    { icon: Upload, label: 'Bulk Upload', href: '/admin/bulk-upload' }, { icon: Bell, label: 'Notifications', href: '/notifications' },
  ],
  front_desk: [
    { icon: LayoutDashboard, label: 'Dashboard', href: '/dashboard' }, { icon: Users, label: 'Patients', href: '/patients' }, { icon: ClipboardList, label: 'Registration', href: '/registration' },
  ...
  practitioner: [
    ...
    { icon: Sparkles, label: 'AI Report', href: '/ai-report' }, { icon: ShieldCheck, label: 'AI Hub', href: '/ai-clinical' }, { icon: Bell, label: 'Notifications', href: '/notifications' },
  ],
  nurse: [
  ...
  lab_technician: [
    ...
    { icon: Upload, label: 'Outside Lab Uploads', href: '/outside-lab' }, { icon: ClipboardList, label: 'Reports', href: '/reports' }, { icon: Bell, label: 'Notifications', href: '/notifications' },
  ],
  pharmacist: [
    { icon: LayoutDashboard, label: 'Dashboard', href: '/dashboard' }, { icon: ClipboardList, label: 'Department Queue', href: '/department-queue' }, { icon: Pill, label: 'Pharmacy / Dispensing', href: '/pharmacy' },
    { icon: ClipboardList, label: 'Inventory', href: '/inventory' }, { icon: FileText, label: 'Stock Alerts', href: '/stock-alerts' }, { icon: Bell, label: 'Notifications', href: '/notifications' },
  ],
  accountant: [
  ...
  patient: [
    { icon: LayoutDashboard, label: 'Dashboard', href: '/dashboard' }, { icon: FileText, label: 'My Portal', href: '/patient-portal' }, { icon: Calendar, label: 'My Appointments', href: '/appointments' },
    { icon: Video, label: 'My Telemedicine', href: '/telemedicine' }, { icon: CreditCard, label: 'My Billing', href: '/billing' }, { icon: Bell, label: 'Notifications', href: '/notifications' },
  ],
};
...
export default function Sidebar() {
  ...
  return (
    <aside className={cn('fixed left-0 top-0 z-40 h-screen sidebar-gradient transition-all duration-300 flex flex-col', collapsed ? 'w-20' : 'w-64')}>
      <div className="flex items-center justify-between p-4 border-b border-sidebar-border">
        {!collapsed ? <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-sidebar-primary flex items-center justify-center"><HeartPulse className="w-6 h-6 text-sidebar-primary-foreground" /></div><div><h1 className="font-heading font-bold text-sidebar-foreground text-lg">MediCare</h1><p className="text-xs text-sidebar-foreground/60">Pro Health System</p></div></div> : <div className="w-10 h-10 rounded-xl bg-sidebar-primary flex items-center justify-center mx-auto"><HeartPulse className="w-6 h-6 text-sidebar-primary-foreground" /></div>}
      </div>
      <nav className="flex-1 p-3 overflow-y-auto scrollbar-none">
        <ul className="space-y-1">
          {navItems.map((item) => {
            ...
            return <li key={item.label + item.href}><Link to={item.href} className={cn(isActive ? 'nav-link-active' : 'nav-link', collapsed && 'justify-center px-2')}><div className="relative"><item.icon className="w-5 h-5 flex-shrink-0" />{isNotif && unreadCount > 0 && <span className="notification-dot" />}</div>{!collapsed && <><span className="flex-1">{item.label}</span>{isNotif && unreadCount > 0 && <span className="bg-critical text-critical-foreground text-xs font-medium px-2 py-0.5 rounded-full">{unreadCount > 9 ? '9+' : unreadCount}</span>}</>}</Link></li>;
          })}
        </ul>
      </nav>
      <div className="p-3 border-t border-sidebar-border">
        {!collapsed && <div className="flex items-center gap-3 px-3 py-2 mb-2"><div className="w-9 h-9 rounded-full bg-sidebar-accent flex items-center justify-center text-sidebar-foreground font-medium">{(user.firstName?.[0] || user.email[0]).toUpperCase()}{(user.lastName?.[0] || '').toUpperCase()}</div><div className="flex-1 min-w-0"><p className="text-sm font-medium text-sidebar-foreground truncate">{user.firstName} {user.lastName}</p><p className="text-xs text-sidebar-foreground/60 capitalize truncate">{user.role.replace('_', ' ')}</p></div></div>}
        <button onClick={logout} className={cn('nav-link w-full text-critical hover:text-critical hover:bg-critical/10', collapsed && 'justify-center px-2')}><LogOut className="w-5 h-5" />{!collapsed && <span>Sign Out</span>}</button>
      </div>
      <button onClick={() => setCollapsed(!collapsed)} className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-card border border-border shadow-md flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">{collapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}</button>
    </aside>
  );
}