...
| Device integration | existing laboratory/imaging surfaces | registry, protocol boundary, lifecycle state machine, isolated heartbeat and governed transport-ledger intake | vendor validation, safety, security, resilience |
...
- `supabase/migrations/20260919070000_hms_role_permission_workflow_boundaries.sql`: converges the broader HMS role/module matrix onto the existing `user_roles`/`app_role` authority, adds a fail-closed facility-module + role-permission assertion boundary, preserves immutable Super Admin access, and routes governed Data Import lifecycle operations through the canonical permission gate. Unsupported department/user permission scopes remain non-authorizing until a concrete scope context exists.
- `Encounters.tsx` now exposes the admission action directly on a successfully completed encounter, using the existing `admit_encounter_workflow` boundary; the superseded floating admission overlay was removed to avoid duplicate admission surfaces. Facility emergency-treatment and Accounts-release settings remain server-governed by the existing facility configuration contract.
...
Device integration now has a server-authoritative lifecycle and transport boundary: administrators onboard devices through a validated RPC, lifecycle transitions are explicit and locked against concurrent edits, every transition requires a reason and enters the existing audit surface, authenticated direct mutation is revoked, heartbeat reporting is isolated to service role, and only active devices can enqueue governed ASTM/HL7/FHIR/DICOM/REST/SOAP messages into the durable transport ledger. The intake path performs payload-integrity and collision checks but does not create canonical clinical results.
